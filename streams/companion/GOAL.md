...
- **Character Forge (Arthur, 2026-07-06):** Fable as character designer — original characters via text → image (GPT Image/codex sub) → 3D → rig → VRM; silhouette-first, 2-3 colors, archetype compression ("low-Kolmogorov characters"), cosplayable, channel-manifest ready. Philosophy + persona silhouette seeds: `notes/character-forge.md`; SOTA pipeline survey: `notes/character-pipeline.md` (in flight). Perfect-sync face donors already local: `data/avatar-models/{hinzka-vroid-v110,blender-vrm-perfect-sync}-*` (52/52 ARKit verified).
