...
- **/lab React cutover landed (same commit):** React 19 + Tailwind v4 `@theme` tokens + local shadcn-style primitives under `public/lab/`; the retired monolithic surface is replaced by the current bundled stylesheet and panel components; the 281-clip library virtualizes via `@tanstack/virtual-core` with stable media nodes; the full Face Mirror behavior (socket/drive/replay/preview/overlay/record/export/RAF) is ported, Mirror X defaults on. Every workflow, ID, global (`__labState` etc.), storage key, and `lab:*` event survives. Gates: `tsc` clean, `bun run build:lab`/`build:vrm` ok, 366 tests / 0 fail, live browser QA on `/lab` (panes, Space, selection, synthetic mirror replay, record/export, telemetry).
...
7. **Sico offline batch**: `AppleMotionOracleBatch` (AVAssetReader, per-frame 2D body+hands with live association policy + 3D) over the 281-clip sico corpus → `data/apple-vision-tracks/` + MediaPipe same-clip comparison vs the 73 `data/body-tracks` v2 tracks. Completed 2026-07-15; see `streams/companion/notes/vision-sico-batch.md`.

**Still open:** deliberate live close-hand/full-body calibration pass (needs Arthur on camera ~2 min; runbook `docs/motion-capture-comparison.md`), WiLoR full run (MANO), and the v2 replay-retirement Xanadu answer. The batch, evidence card, and calibration-weight outcomes are recorded in `streams/companion/notes/overnight-program-2026-07-15.md`.

#### CUDA/TensorRT/OpenAI Triton provider contract — Arthur decision

Use a **single semantic/model source of truth**, not a mandatory interchange format. Arthur explicitly does not want ONNX to become architectural authority. Keep one canonical PyTorch model/reference implementation plus one machine-readable contract containing tensor shapes/layouts/dtypes, preprocessing/postprocessing, joint vocabularies, coordinate/chirality rules, precision policy, shape profiles, and golden inputs/outputs. Backend artifacts are generated and disposable; they never become independently edited model implementations.

Moondream Photon/Kestrel is a product-architecture reference, not our open-infrastructure base: one public model/scheduler/runtime surface dispatches through one logical kernel API, but its optimized CUDA/Metal implementations ship in separately pinned proprietary kernel/native packages. Copy the narrow API, scheduler, version pinning, and parity discipline—not the openness claim.

Generate two optimized products from the canonical source:

- **NVIDIA batch lane:** PyTorch export/Dynamo directly into Torch-TensorRT or measured Inductor; CUDA Graphs and explicit batching; OpenAI Triton language only for profiled preprocessing, decoder/NMS, unsupported operations, or profitable fusions. Optimize throughput across many clips.
- **Apple realtime lane:** convert the same canonical graph to Core ML where supported; use Metal/MLX or MPSGraph only for measured gaps. Optimize warmed batch-1 latency and zero-copy camera buffers. Keep MLX as a benchmark challenger for a literal shared high-level graph across Metal and CUDA, but do not choose it until it matches the required NVIDIA throughput and Mac latency; its CUDA backend does not provide TensorRT or ANE.

OpenAI Triton language does **not** currently provide a production Apple Metal backend. Do not force one kernel implementation across NVIDIA and Apple. The single source of truth is the canonical model+contract+goldens; backend kernels are replaceable generated/optimized leaves.

Generate the stable C ABI/header from the same contract. Use an opaque context, versioned POD structs, caller-owned buffers, explicit host/device inputs, an optional backend-native stream/queue handle, async batched inference, and status codes. No STL, exceptions, hidden allocation, JSON, Apple-native types, or Companion transport in the hot ABI. Backend adapters convert results to `companion.motion-oracle/1` or `companion.motion-oracle-3d/1` on loopback UDP `49982`.

The eventual “SGLang-like” layer is a shared motion runtime above both executors: warm model registry, NVIDIA dynamic/offline batching, Mac latest-frame/drop-while-busy scheduling, cancellation, observability, and typed output arbitration. SGLang is a design reference, not a dependency. CI must run identical goldens through reference, NVIDIA, and Apple paths; compare named outputs with per-field tolerances and fail artifact publication on drift.

#### Open compiler/runtime bake-off — no winner preselected

Benchmark these challengers before committing the canonical compiler:

1. **Burn/CubeCL:** strongest literal one-source experiment. One Rust model/kernel stack advertises CUDA, ROCm, Metal, Vulkan, WebGPU, and CPU, with a real WASM/WebGPU browser example. Risks: pre-release maturity, no ANE, and unknown performance/operator coverage for this graph.
2. **TVM Unity/Relax:** strongest fully open compiler/runtime experiment. Direct PyTorch-export importer; CUDA, Metal, WebGPU with WASM host; layered Relax/TIR design and reproducible target artifacts. Risks: highest compiler/schedule/build ownership.
3. **TileLang:** benchmark as a kernel DSL, not the model/runtime owner. It is MIT/open and has CUDA, real-but-young Metal execution, autotuning, and WebGPU codegen. Browser graduation requires an actual JS/WASM host execution path; generated WGSL alone is failure. CUDA is currently its strongest target.
4. **MAX/Mojo:** benchmark native full-graph NVIDIA/AMD/Apple performance only. It has no browser/WASM/WebGPU or ANE path, and the load-bearing compiler/runtime SDK remains Community-Licensed/non-open despite substantial open kernels/model/serving code. It cannot become the canonical open infrastructure without those boundaries changing.
5. **Vendor/native baselines:** PyTorch/Inductor or TensorRT on NVIDIA, Core ML/Metal on Apple, and a WASM CPU browser floor. A challenger graduates only by matching semantics and improving measured performance or materially reducing system burden.

Use one representative mobile landmark slice first: depthwise 3×3 → SiLU → pointwise 1×1 → SiLU → global pool → linear → `[B,75,3]` (33 body + 21 left + 21 right), then run the full model only for finalists. NVIDIA batches `1/8/32/128`; Apple and browser batch `1`. Record correctness before speed, p50/p95, throughput, cold/compiled startup, memory, artifact size, CPU submission/GPU time, fallback/operator assignment, and generated source/tuning config. Browser matrix must include Chrome and Safari WebGPU plus WASM-SIMD fallback; test real execution, camera preprocessing, device loss, and cross-origin-isolation behavior.
...
- **Motion Lab studio `/lab`**: four-pane shell, current source stylesheet `public/lab/lab.css` + vendored Lucide sprite (laws in `notes/lab-design-system.md`), multi-track editor timeline (scrub/zoom/mute-gates), sico library (281 clips, synced video), pipeline monitor (SSE job queue).
- **Transfer stack**: pose (`public/pose-transfer.ts`, direction-vector retarget), hands (`hand-transfer.ts`, 30 finger bones), face (`face-transfer.ts` + `face-mapping.ts`: crop-follow for camera moves, response curves, neutral calibration, per-class filtering — measured composite 61→66.5, jerk −31%), skeleton overlay on the reference video, capture lanes in `vrm-body.ts`, v2 body-tracks (world+image+hands). **Provider-track replay is now the DEFAULT motion source (GPU → Vision → v2 fallback); the prior per-clip captured-replay default is superseded by `streams/companion/notes/overnight-program-2026-07-15.md`.**
