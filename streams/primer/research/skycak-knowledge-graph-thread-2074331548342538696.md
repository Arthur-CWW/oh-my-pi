# Justin Skycak — "The knowledge graph is the main ingredient in our secret sauce" (X thread)

- **Source**: https://x.com/justinskycak/status/2074331548342538696
- **Extracted**: 2026-07-10 (via authenticated cmux browser session)
- **Author**: Justin Skycak (@justinskycak)
- **Posted**: 2026-07-07T03:15:54Z (rendered as "1:15 PM · Jul 7, 2026")
- **Engagement at capture**: 254.4K views · 33 replies · 171 reposts · 1.4K likes · 2.4K bookmarks
- **Thread structure**: single long-form post. The author continued the thread ~3h later in a chained follow-up: https://x.com/justinskycak/status/2074376893067911436 (extracted separately in `skycak-knowledge-graph-thread-2074376893067911436.md`).

## Thread

### Tweet 1 — @justinskycak
Permalink: https://x.com/justinskycak/status/2074331548342538696
...
## Quote Tweet (embedded in Tweet 1)

- **Alex Smith** (@ninja_maths), Jul 7 (rendered 2026-07-06T17:06:21Z):
  ...
  >
  > This knowledge graph spans 3,000 math topics, from 4th grade to the university level, providing the perfect basis for mastery learning.  x.com/unrealNadja/st…

## Media

- Image embedded in Tweet 1: `https://pbs.twimg.com/media/HMj1_YVXoAAt_AW?format=jpg&name=medium` — alt text is the generic "Image"; not visually inspected during capture. [AMBIGUOUS] Context (the @ninja_maths quote about a "knowledge graph [spanning] 3,000 math topics") suggests it is the Math Academy knowledge-graph visualization, but this is inferred from surrounding text, not from the image itself.

## Replies

**None rendered.** The post's counter showed **33 replies** at capture time, but the conversation timeline below the reply composer never mounted in the capture environment. Exactly what rendered: the focal post (complete), its embedded quote tweet, the engagement stats row ("254.4K Views · 33 · 171 · 1.4K · 2.4K"), the reply sort selector ("Relevant") and "View quotes" link, the "Post your reply" composer, and a loading spinner that never resolved.

Technical cause: the authenticated cmux surface is an offscreen WKWebView reporting `document.visibilityState === "hidden"`; x.com defers the conversation-timeline fetch until the document is visible, and engine-level occlusion throttling kept the fetch pending despite native scrolling, reloads, long settles, and page-world visibility overrides. Loading replies would require the surface to be actually visible on screen (focus-stealing, which was out of scope for this read-only capture).

## Omissions / Limitations

- 33 replies exist but 0 were retrievable (see above).
- The author's own thread continuation (status 2074376893067911436) would normally appear in this conversation; it was captured from its own permalink instead.
- Embedded image not downloaded; alt text was the generic "Image".
