...
- **Reader viewer is persistently supervised** at `http://meltdown.localhost:1355` (current supervisor PID in `data/meltdown-reader/dev-up.pid`; health `/api/health`; one log `data/meltdown-reader/errors.log`). Runtime lives in nested READER `scripts/dev-supervisor.mjs`: atomic PID/identity lock, backend + Vite/portless detached groups, paired restart, TERM→KILL escalation, HMR, safe static-alias takeover. Start after reboot/session loss with `cd READER && nohup bun scripts/dev-supervisor.mjs >/dev/null 2>&1 &`; stop with `/bin/kill -TERM "$(cat data/meltdown-reader/dev-up.pid)"` from outer repo. Session-level only: survives shell exit, not reboot.
