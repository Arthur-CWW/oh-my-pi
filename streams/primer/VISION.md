...
10. **Media reading follows the video-learning grammar.** Arthur's 2026-07-22 reference is Bilibili/YouTube: video is the central object; a large, independently scrolling transcript sits beside it even in a cmux half-pane. The transcript is not a narrow article with a floating thumbnail.
11. **Pinyin is a glyph-aligned interaction layer, not a loose sentence gloss.** Every syllable sits directly above its glyph on a shared baseline; missing syllables reserve their cell. The glyph itself is the seek/replay control—never an adjacent duplicate play button. Digits and decimals are real spoken glyphs (`2024`, `1.4`), so they receive Mandarin readings and timing too. Media is local and preloaded for immediate read-along replay.
12. **Reviewed Anki state is prior knowledge, not a second queue.** Read AnkiConnect without mutation; snapshot card/note identity, full current scheduling state, complete review history, selected Mandarin/Cantonese sentence/audio fields, tags, and flags. Treat reviewed terms as known generation context. A marked/flagged card remains starred. Never reset or reinterpret Anki history as fresh Primer reviews.
13. **One incidental unknown per generated sentence.** The target item is allowed; beyond it, at most one distinct unknown token. More is a regeneration failure, not merely a lower difficulty score.
