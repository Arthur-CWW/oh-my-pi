...
- **Model routing.** Never spawn Fable subagents (orchestrator-only; hard-guarded in the model resolver). **Never route to Terra** (Arthur, 2026-07-15: "not pareto-efficient at anything"). Lane assignments are NOT cached here: resolve each responsibility from live config and `docs/fable/routing-doctrine.md`; posture history lives in `docs/fable/agent-stack-consolidation.md`. Every `task` spawn MUST pass the responsibility's explicit provider/model:effort selector; omission is invalid and MUST be rejected before launch. Sol escalation requires a packet-local reason rather than task size.
