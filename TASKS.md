...
| T-2026-06-27-004 | Build clean-room local-first AI companion realtime testbed | Codex | Active; canonical continuation: `streams/companion/HANDOFF-BEHAVIOR.md`. 2026-07-22 milestones: VRM delta/lateral-frame source repairs, deformation-gated basis hillclimb, compact `animation:probe` API/CLI, focused `/animation` route, persistent camera, simplified Motion Lab, and systematic rig-transfer QA shipped. Rig proof: `local/rig-clinic-proof/coordinate-scan/` (SQLite/NumPy + typed Rust scanners, native/WASM reviewer, anomaly sheets, scanner parity, real diagnostic variants, local-VLM falsification). The Qwen2.5-VL-3B local triage gate failed honestly (2/4 defect-class hits), so VLM output remains non-authoritative human-queue evidence only. |
...
| T-2026-07-18-001 | Devices workstream: one secure Android/iPhone execution core | Arthur conversation / Devices | Active. Charter `streams/devices/GOAL.md`; repo-owned `packages/device-control`; typed OMP tools; named gateway/process leases. Mac primary plus Xiaomi Android lab deployed behind loopback + tailnet API; USB-first ADB with verified wireless fallback; iPhone CoreDevice iOS 27 HID gate with WDA fallback. |
