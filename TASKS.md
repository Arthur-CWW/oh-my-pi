...
| T-2026-06-09-025 | Capture Jimeng reference/persona/video/canvas API flows | Conversation | Use `jimeng-browser-proxy` skill and passive CDP captures for first-frame image-to-video payloads using committed ImageX URIs, image-to-image, subject/persona create/update/generate_voice, voice clone submit/query, pose/style/depth/canny reference controls, multimodal video, lip sync generation, VOD upload, canvas edits, asset library, and explore/template APIs. |
