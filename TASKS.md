...
| T-2026-06-09-024 | Reverse/catalog useful Jimeng GenAI frontend APIs | Codex | Build a truthful, tested `jimeng-browser-proxy` CLI surface first; defer daemon/session-refresh orchestration until the API contracts settle. Current slices support workbench text-to-image submit + asset-list polling, non-generating config probes, built-in voice library replay, direct TTS MP3 generation, upload-token retrieval, local ImageX image upload to provider URI, no-spend saved subject/persona listing, no-spend subject/persona creation from uploaded or existing provider images, no-spend reference-image description/face inspection, no-spend ControlNet pose/depth/canny preview with preview PNG proof, no-spend object/saliency segmentation with mask PNG proof, local-upload-backed first-frame image-to-video with MP4 proof, local VOD video upload to `vid`/`tos-cn-v-*` provider references, dry-run-proved first/end-frame `frames2video` patching, dry-run VOD lip-sync provider-input planning, dry-run image/avatar lip-sync provider-input planning, no-spend lip-sync/digital-human model config, no-spend Explore/template mining, no-spend short-video Explore reference mining, no-spend `/mweb/v1/feed_short_video` overseas/reference mining, no-spend signed CapCut commercial template category catalog, and no-session public CapCut ratio/scene metadata catalog. Next focus: frontend lip-sync submit capture/compare, end-frame live proof, style/reference payload controls, CapCut template row/search/collection payload capture, digital-human live generation, voice clone, and subject/persona update/delete/generate_voice APIs with proof artifacts and command logs. |
...
| T-2026-06-09-048 | Add CapCut commercial template category CLI support | 2026-06-10 | Added `jimeng-browser-proxy capcut-categories` for no-spend signed `/lv/v1/cc_web/plane/get_categories`, frontend signer tests, docs, and proof under `data/jimeng-lab/proof-20260610-capcut-categories/`; CapCut template row/search/collection endpoints still need real UI payload capture. |
