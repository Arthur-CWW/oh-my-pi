...
- [@User] Promote Kagi as default `web_search` provider in Effect entry, and continue Puppeteer reverse-engineering captures.
  - Result: Added `src/effect/kagi-search.ts` with lazy-loaded Kagi client, SSE tag payload parsing (`top-content-unique`, `search`, `error`), HTML result extraction, and option mapping (`lens`, `recencyFilter`, `domainFilter`); wired `src/effect/index.ts` to prefer Kagi and override legacy `web_search` at cutover; added tests `tests/kagi-search-effect.test.ts` + updated `tests/effect-index.test.ts`; refreshed probe script date handling and documented new findings in `packages/kagi/notes/api-quirks.md`.
