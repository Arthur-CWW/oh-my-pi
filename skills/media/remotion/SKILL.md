...
## Monorepo Layout & Command Conventions

In this workspace:
- The Remotion renderer package is located under `packages/remotion-renderer/`.
- **DO NOT** scaffold a new project with `npx create-video` or similar commands unless explicitly asked to create a standalone folder. Instead, extend and add compositions to the existing workspace package.
- Place new compositions under `packages/remotion-renderer/src/compositions/` and register them in `packages/remotion-renderer/src/Root.tsx`.
- **Assets handling**: The standard Remotion skill suggests placing assets in `public/` and using `staticFile()`. However, in this monorepo, our custom renderer script (`packages/remotion-renderer/src/render.ts`) parses arbitrary local paths (e.g. image plates, audio tracks) and embeds them as base64 data URLs via `toRenderableUrl`. You can pass absolute or relative paths directly to assets. For standard Remotion Studio/CLI commands, assets should be placed in `packages/remotion-renderer/public/` and referenced using `staticFile()`.
- To run/test/render, use the following root package scripts:
  - **Typecheck**: `bun run remotion-renderer:typecheck`
  - **Test**: `bun run remotion-renderer:test`
  - **Render**: `bun run remotion-renderer:render` (invokes `packages/remotion-renderer/src/render.ts`)
  - **Studio**: `bun --cwd packages/remotion-renderer x remotion studio` to launch Remotion Studio.
  - **Still**: `bun --cwd packages/remotion-renderer x remotion still [composition-id] --scale=0.25 --frame=30` to check a single frame.
