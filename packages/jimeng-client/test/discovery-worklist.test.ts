...
describe("Jimeng discovery worklist", () => {
  test("prioritizes uncovered read-safe endpoints and hides already-covered captures by default", () => {
    ...
    try {
      ...
      expect(preProcess?.known_status).toBe("dry_run_only")
      expect(preProcess?.recommended_action).toBe("approval_or_disposable_fixture")
      expect(preProcess?.reason).toContain("Known dry-run-only endpoint")
      const preProcessResult = worklist.items.find((item) => item.endpoint === "/mweb/v1/video_generate/mget_pre_process_result")
      expect(preProcessResult?.known_status).toBe("dry_run_only")
      expect(preProcessResult?.recommended_action).toBe("approval_or_disposable_fixture")
      expect(preProcessResult?.reason).toContain("Known dry-run-only endpoint")
