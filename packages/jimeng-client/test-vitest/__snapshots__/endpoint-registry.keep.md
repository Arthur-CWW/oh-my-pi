...
3. Lip-sync / digital human - `L1 /mweb/v1/video_generate/pre_process` - blocked command=video-preprocess-plan/request-plan-compare/submitJimengVideoPreprocess - Blocked at provider task creation: request bodies are modeled and typed service/cassette replay exists, and background CDP preflight now proves the true digital-human route, voice picker, and script/action editor population. A live pre_process payload still requires approved role/avatar upload or preselected avatar state that enables generation.
...
- `/mweb/v1/video_generate/get_switch_model_queue_info` - blocked - No-spend probes with empty, model_req_key, model_req_keys, and scene bodies returned ret=1000 invalid parameter; capture the exact frontend switch-model queue body before promotion.
  ...
  - Next probe: Capture the frontend switch-model queue request from the lip-sync/video UI and replay the exact body through endpoint-probe record/replay.
- `/mweb/v1/video_generate/pre_process` - blocked command=video-preprocess-plan/request-plan-compare/submitJimengVideoPreprocess - Blocked at provider task creation: request bodies are modeled and typed service/cassette replay exists, and background CDP preflight now proves the true digital-human route, voice picker, and script/action editor population. A live pre_process payload still requires approved role/avatar upload or preselected avatar state that enables generation.
