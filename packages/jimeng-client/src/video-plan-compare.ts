...
export interface JimengVideoDirectCompareDifference {
  path: string
...
export interface JimengVideoDirectCompareCandidate {
  index: number
  ...
  differences: JimengVideoDirectCompareDifference[]
}

export interface JimengVideoDirectCompareResult {
  match: boolean
  plan_endpoint: string
  plan_model_req_key: string | null
  candidate_count: number
  candidates: JimengVideoDirectCompareCandidate[]
}
...
interface ParsedVideoDirectBody {
...
export function compareJimengVideoDirectPlanWithRawNetwork(input: {
  dryRunPlanText: string
  rawNetworkText: string
}): JimengVideoDirectCompareResult {
  return compareJimengVideoDirectPlanWithRequests({
    dryRunPlanText: input.dryRunPlanText,
    requests: extractVideoDirectSubmitRequestsFromRawNetwork(input.rawNetworkText),
  })
}

export function compareJimengVideoDirectPlanWithCaptureTemplate(input: {
  dryRunPlanText: string
  captureTemplateText: string
}): JimengVideoDirectCompareResult {
  return compareJimengVideoDirectPlanWithRequests({
    dryRunPlanText: input.dryRunPlanText,
    requests: extractVideoDirectSubmitRequestsFromCaptureTemplate(input.captureTemplateText),
  })
}

export function compareJimengVideoDirectPlanWithRequests(input: {
  dryRunPlanText: string
  requests: CapturedSubmitRequest[]
}): JimengVideoDirectCompareResult {
  const expected = parseExpectedDirectVideoBody(input.dryRunPlanText)
...
export function summarizeJimengVideoDirectCompare(result: JimengVideoDirectCompareResult): JsonObject {
  return {
    match: result.match,
...
function compareCapturedRequest(expected: ParsedVideoDirectBody, request: CapturedSubmitRequest, index: number): JimengVideoDirectCompareCandidate {
  const captured = parseCapturedDirectVideoBody(request.postData, pathnameValue(request.url))
  const differences: JimengVideoDirectCompareDifference[] = []
  ...
  matchedPathCount += compareOptionalObject(expected.videoTaskExtra, captured.videoTaskExtra, "video_task_extra", differences)
  ...
  return {
    ...
    video_input_match: !differences.some((difference) => difference.path.startsWith("videoInput.") || difference.path === "text_to_video_params.video_aspect_ratio"),
    metrics_match: !differences.some((difference) => difference.path.startsWith("metrics_extra.")),
    video_task_extra_match: !differences.some((difference) => difference.path.startsWith("video_task_extra.")),
...
function parseExpectedDirectVideoBody(text: string): ParsedVideoDirectBody {
  const root = parseJsonObject(text, "video direct dry-run plan")
  ...
  return parseDirectVideoBody(request, endpoint, "video direct dry-run request")
}

function parseCapturedDirectVideoBody(postData: string, endpoint: string): ParsedVideoDirectBody {
  return parseDirectVideoBody(parseJsonObject(postData, "captured video direct submit body"), endpoint, "captured video direct submit body")
}

function parseDirectVideoBody(body: JsonObject, endpoint: string, operation: string): ParsedVideoDirectBody {
  ...
  const videoTaskExtra = parseJsonObjectString(genVideo?.video_task_extra, `${operation} video_task_extra`, false)
  return {
    submitId: stringValue(body.submit_id),
    endpoint,
    rootModel: stringValue(recordValue(body.extend)?.root_model),
...
function extractVideoDirectSubmitRequestsFromRawNetwork(text: string): CapturedSubmitRequest[] {
  ...
  for (const [index, line] of lines.entries()) {
    ...
    if (!url || !postData || pathnameValue(url) !== JIMENG_VIDEO_DIRECT_ENDPOINT) continue
    requests.push({
...
function extractVideoDirectSubmitRequestsFromCaptureTemplate(text: string): CapturedSubmitRequest[] {
  ...
  return parsed.entries.flatMap((entry, index) => {
    if (entry.kind !== "request" || !entry.url || !entry.postData || pathnameValue(entry.url) !== JIMENG_VIDEO_DIRECT_ENDPOINT) return []
...
function compareVideoInput(expected: JsonObject | null, actual: JsonObject | null, differences: JimengVideoDirectCompareDifference[]): number {
  if (!expected || !actual) return compareOptionalObject(expected, actual, "videoInput", differences)
  let matched = 0
  for (const key of ["prompt", "video_mode", "fps", "duration_ms", "resolution", "seed"]) {
    matched += compareScalar(expected[key], actual[key], `videoInput.${key}`, differences)
  }
  matched += compareOptionalField(expected, actual, "first_frame_image", differences)
  matched += compareOptionalField(expected, actual, "end_frame_image", differences)
  return matched
}

function compareOptionalField(expected: JsonObject, actual: JsonObject, key: string, differences: JimengVideoDirectCompareDifference[]): number {
  ...
  return compareScalar(expected[key], actual[key], `videoInput.${key}`, differences)
}
...
function compareSubset(expected: JsonValue, actual: JsonValue | null, pathKey: string, differences: JimengVideoDirectCompareDifference[]): number {
...
function compareScalar(expected: JsonValue | undefined | string | null, actual: JsonValue | undefined | string | null, pathKey: string, differences: JimengVideoDirectCompareDifference[]): number {
...
function parseJsonObject(text: string, operation: string): JsonObject {
  ...
  throw jimengError({
    category: "validation",
    code: "VIDEO_DIRECT_JSON_OBJECT_REQUIRED",
...
function parseUnknownJson(text: string, operation: string): unknown {
  try {
    ...
    throw jimengError({
      category: "validation",
      code: "VIDEO_DIRECT_JSON_PARSE_FAILED",
...
function parseJsonObjectString(value: JsonValue | undefined, operation: string, required: boolean): JsonObject | null {
  if (typeof value !== "string") {
    ...
    throw jimengError({
      category: "validation",
      code: "VIDEO_DIRECT_JSON_STRING_REQUIRED",
...
function decodeCaptureContract<A>(schema: Schema.Decoder<A>, value: unknown, operation: string): A {
  try {
    ...
    throw jimengError({
      category: "validation",
      code: "VIDEO_DIRECT_CAPTURE_CONTRACT_CHANGED",
