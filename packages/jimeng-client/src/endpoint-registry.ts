...
const KNOWN_ENDPOINTS: JimengDiscoveryKnownEndpoint[] = [
  ...
  known("/mweb/v1/video_generate/pre_process", "blocked", "video-preprocess-plan/request-plan-compare/submitJimengVideoPreprocess", "Blocked at live lip-sync entry state: pre-process request bodies are modeled and typed service/cassette replay exists, but the current ?type=lip_sync route still renders a generic composer with no confirmed voice-picker controls, and the first browser-backed lip-sync attempt failed with JIMENG_LIP_SYNC_VOICE_OPTION_MISSING."),
