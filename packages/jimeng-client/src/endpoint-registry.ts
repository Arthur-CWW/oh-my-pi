...
const PACKET_QUEUE: JimengDiscoveryPacketQueueItem[] = [
  ...
  },
  {
    ...
    blocker: "Background CDP preflight now confirms the digital-human route, voice picker, and TipTap script/action editor population without upload or submit. Provider pre-process/generate payloads still require approved role/avatar upload or preselected avatar state that enables generation and advances toward provider task creation.",
...
const KEEP_GAP_AUDIT_BY_ENDPOINT: Record<string, JimengDiscoveryEndpointAudit> = {
  ...
  "/mweb/v1/dreamina_subject/generate_voice": {
    ...
    nextProbe: "Capture a subject generate-voice UI submit and compare it with subject-generate-voice dry-run using request-plan-compare, then record/replay generateJimengSubjectVoice after explicit approval because it can consume quota.",
  },
  "/mweb/v1/voice/submit_task": {
    evidence: [
      "docs/qa/jimeng-request-plan-compare-20260611.md",
    ...
    nextProbe: "Capture a voice-clone submit UI request with disposable source audio and compare the dry-run plan, then record/replay submitJimengVoiceClone after explicit approval because it can create account assets.",
  },
  "/mweb/v1/voice/query_task": {
    evidence: [
      "docs/qa/jimeng-request-plan-compare-20260611.md",
    ...
    nextProbe: "Use a real task id from an approved voice-clone submit flow, then record/replay queryJimengVoiceTasks as a cassette-backed read.",
  },
  "/mweb/v1/voice/update": {
    evidence: [
      "docs/qa/jimeng-request-plan-compare-20260611.md",
    ...
    nextProbe: "Capture or create a disposable cloned voice asset, compare voice-clone-update dry-run against the UI request, then record/replay updateJimengClonedVoice only after explicit mutation approval.",
  },
  "/mweb/v1/voice/delete": {
    evidence: [
      "docs/qa/jimeng-request-plan-compare-20260611.md",
    ...
    nextProbe: "Capture or create a disposable cloned voice asset, compare voice-clone-delete dry-run against the UI request, then record/replay deleteJimengClonedVoice only after explicit mutation approval.",
  },
  "/mweb/v1/feed": {
    evidence: [
  ...
  "/mweb/v1/mix_audio_video": {
    evidence: [
      ...
      "docs/qa/jimeng-mix-audio-client-20260613.md",
      "data/jimeng-lab/proof-20260611-static-locate-media-helper-blockers/",
      "data/jimeng-lab/proof-20260611-static-inventory-media-helper-blockers/",
...
const KNOWN_ENDPOINTS: JimengDiscoveryKnownEndpoint[] = [
  known("/mweb/v1/aigc_draft/generate", "implemented", "text2image-plan/text2image-compare/text2video-plan/omni-video-plan/text2video-compare/omni-video-compare/generation-contract/executeJimengVideoDirectSubmit/executeJimengText2ImageSubmit/text2video/image2video/frames2video/lip-sync", "Unified generation submit for useful image/video workflows is implemented: direct text/video submit/poll/download and browser-backed text/image submit/poll/download are live-proven, saved generation proofs are schema-validated, and dry-run/compare gates cover direct image/video, first/end-frame, and Seedance omni-reference planners. Remaining pure direct text-to-image signer/a_bogus parity is an endpoint-level follow-up, not a blocker for user-facing generation parity."),
  known("/mweb/v1/execute_generate_audit", "blocked", "generate-audit-plan/request-plan-compare/executeJimengGenerateAudit", "Blocked on live frontend context: generate-audit-plan and executeJimengGenerateAudit cover material transform and replay/cassette behavior, but there is no passive capture of the frontend /mweb/v1/execute_generate_audit request around a real generation submit proving the complete top-level context and provider response."),
  ...
  known("/mweb/v1/mget_story", "blocked", "story-records", "No-spend story detail lookup by story_id_list is typed and replay-covered, but live proof is blocked until a public profile or UI capture yields a non-empty story list and real story_id_list values."),
  ...
  known("/mweb/v1/video_generate/pre_process", "blocked", "video-preprocess-plan/request-plan-compare/submitJimengVideoPreprocess", "Blocked at provider task creation: request bodies are modeled and typed service/cassette replay exists, and background CDP preflight now proves the true digital-human route, voice picker, and script/action editor population. A live pre_process payload still requires approved role/avatar upload or preselected avatar state that enables generation."),
  known("/mweb/v1/video_generate/mget_pre_process_result", "blocked", "video-preprocess-query-plan/request-plan-compare/fetchJimengVideoPreprocessResults", "Blocked on upstream pre_process task ids from the true lip-sync/digital-human workbench: submit_id_list lookup is modeled and typed service/cassette replay exists, but no live task ids can be produced until an approved role/avatar upload or submit flow creates provider task state."),
  ...
  known("/mweb/v1/voice/submit_task", "blocked", "voice-clone-submit/request-plan-compare/submitJimengVoiceClone", "Voice clone submit request and response summaries are typed with cassette replay tests; live clone creation is blocked until a disposable source-audio flow is captured and explicit approval allows account asset creation or quota use."),
  known("/mweb/v1/voice/query_task", "blocked", "voice-clone-query/request-plan-compare/queryJimengVoiceTasks", "Voice task query request and summaries are typed with cassette replay tests; live proof is blocked until a real task id exists from an approved voice-clone submit flow."),
  known("/mweb/v1/voice/update", "blocked", "voice-clone-update/request-plan-compare/updateJimengClonedVoice", "Voice update request and summaries are typed with cassette replay tests; live replay is blocked until a disposable cloned voice asset exists and explicit mutation approval is granted."),
  known("/mweb/v1/voice/delete", "blocked", "voice-clone-delete/request-plan-compare/deleteJimengClonedVoice", "Voice delete request and summaries are typed with cassette replay tests; live replay is blocked until a disposable cloned voice asset exists and explicit mutation approval is granted."),
  known("/mweb/v1/feed", "implemented", "voices", "Built-in voice library replay is implemented for captured signed feed requests."),
