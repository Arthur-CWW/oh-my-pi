...
const KNOWN_ENDPOINTS: JimengDiscoveryKnownEndpoint[] = [
  ...
  known("/mweb/v1/voice/submit_task", "partial", "voice-clone-submit/request-plan-compare/submitJimengVoiceClone", "Voice clone submit request and response summaries are typed with cassette replay tests; live replay remains approval-gated because it may create account assets or consume quota."),
  known("/mweb/v1/voice/query_task", "partial", "voice-clone-query/request-plan-compare/queryJimengVoiceTasks", "Voice task query request and summaries are typed with cassette replay tests; live proof needs a real task id from an approved submit flow."),
  known("/mweb/v1/voice/update", "partial", "voice-clone-update/request-plan-compare/updateJimengClonedVoice", "Voice update request and summaries are typed with cassette replay tests; live replay remains approval-gated because it mutates cloned voice assets."),
  known("/mweb/v1/voice/delete", "partial", "voice-clone-delete/request-plan-compare/deleteJimengClonedVoice", "Voice delete request and summaries are typed with cassette replay tests; live replay remains approval-gated because it mutates cloned voice assets."),
  ...
  known("/mweb/v1/dreamina_subject/generate_voice", "partial", "subject-generate-voice/request-plan-compare/generateJimengSubjectVoice", "Subject voice request and audio summary are typed; live replay remains approval-gated because voice generation may consume quota."),
