...
function normalizeSession(value) {
  ...
  const labelOrRow = (keys, fallback = "") => {
...
function hashForSession(sessionId) {
  return `#${encodeURIComponent(sessionId)}`;
}
function selectSessionId(sessions, hash, previousId = null) {
  const ids = new Set(sessions.map((session) => session.sessionId));
  const fromHash = sessionIdFromHash(hash);
  if (fromHash && ids.has(fromHash))
    return fromHash;
...
`))}</code></pre>`);
    ...
    while (index < lines.length) {
      const current = lines[index] ?? "";
      if (current.trim() === "" || /^\s*```/.test(current) || /^\s{0,3}#{1,6}\s+/.test(current) || /^\s*[-*+]\s+/.test(current) || /^\s*\d+[.)]\s+/.test(current)) {
        break;
      }
...
function safeHref(value) {
  ...
  if (!href || /[\u0000-\u001f]/.test(href))
    return null;
  if (/^[a-z][a-z\d+.-]*:/i.test(href) && !/^(?:https?:|mailto:|history:|omp:)/i.test(href))
    return null;
...
function getElement(id) {
  ...
  if (!element)
    throw new Error(`watch UI is missing #${id}`);
  return element;
}
function getDom() {
  const layout = document.querySelector(".watch-layout");
  const rail = getElement("session-list");
  if (!layout)
    throw new Error("watch UI is missing .watch-layout");
  return {
    layout,
    rail,
...
function renderLabelFields(target, session) {
...
async function fetchStateDoc(sessionId) {
  ...
  if (response.status === 404)
    return null;
  if (!response.ok)
    throw new Error(`state document request failed (${response.status})`);
...
function startWatchApp() {
  ...
  const loadDocument = async (session, restoreScrollTop) => {
    ...
    try {
      ...
      if (currentRequest !== requestNumber || selectedId !== session.sessionId)
        return;
      if (markdown === null) {
        ...
        if (!dom.doc.innerHTML)
          dom.doc.innerHTML = '<p class="doc-placeholder">State document is empty.</p>';
      }
  ...
  const refresh = async () => {
    ...
    try {
      ...
      if (session)
        await loadDocument(session, scrollTop);
  ...
  window.addEventListener("hashchange", () => {
    handleRoute().catch(reportClientError);
  });
