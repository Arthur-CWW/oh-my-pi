...
3. If a logged-in or headed browser must be controlled on Arthur's Mac, prefer CuaDriver (`macos-computer-use`) so the target window can be inspected and acted on without raising or stealing focus.
4. Use this repo's Helium/CDP browser-use profile when the task specifically needs DOM execution, CDP network capture, cookies, protocol-level API reversing, frontend provider adapters such as `llm_frontend_browser`, or replay debugging against an existing logged-in browser profile.
...
For visual/GUI browser automation, use CuaDriver through the `macos-computer-use` skill. CuaDriver owns the private macOS/SkyLight behavior; do not reimplement that in this repo.

For protocol automation, use the clean-room browser-use package in `packages/browser-use`. It defaults to Helium on macOS, launches via `open -g -na`, and creates new tabs through CDP background targets.
