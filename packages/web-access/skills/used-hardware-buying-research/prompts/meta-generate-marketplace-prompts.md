...
Use this with ChatGPT Pro / Codex / another planning model to generate marketplace-specific browser-use prompts for any used-hardware buying task.

```text
You are generating prompts for browser-use agents that will perform read-only used-hardware buying research.
...
4. A `verification-prompt.md` prompt for a second browser-use pass that verifies every recommended link.
...
For every generated browser-use prompt, require:
