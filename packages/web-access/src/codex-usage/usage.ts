...
async function loadAuthCredentials(): Promise<{ accessToken: string; accountId: string }> {
  const auth = await readJsonObject(PI_AUTH_FILE)
  ...
  if (!accessToken || !accountId) throw new Error(`${MISSING_AUTH_ERROR} in ${PI_AUTH_FILE}`)
...
export async function getUsage(modelId: string | undefined): Promise<UsageSnapshot> {
  const [data, accountLabel] = await Promise.all([
    requestUsage(),
    getAccountLabel().catch(() => null),
  ])

  const bucket = selectedBucket(data, modelId)
