...
function printSessions(sessions: FrontendSessionRecord[]): void {
  ...
  for (const session of sessions) {
    ...
    if (session.outputPath) console.log(`  Output file: ${session.outputPath}`)
    if (session.blockerReason) console.log(`  Blocker: ${truncate(session.blockerReason, 160)}`)
    if (session.recoveryStep) console.log(`  Recovery: ${truncate(session.recoveryStep, 220)}`)
