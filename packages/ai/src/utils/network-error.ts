...
export interface ProviderClassifierRefusalInput {
...
/** Return the provider classifier category carried by a structured refusal. */
export function getProviderClassifierRefusalCategory(input: ProviderClassifierRefusalInput): string | undefined {
	if (input.stopReason !== "error" || input.stopDetails?.type !== "refusal") return undefined;
	const category = input.stopDetails.category;
