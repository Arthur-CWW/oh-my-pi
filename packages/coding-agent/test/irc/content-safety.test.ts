...
import { IRC_BODY_MAX_CHARS, IRC_BROADCAST_BODY_MAX_CHARS, IrcMessageLengthError } from "../../src/irc/irc-limits";
...
describe("IrcExternalBus content safety", () => {
	it("refuses an oversized cross-session message and never persists it", () => {
		withExternalBus(bus => {
			...
			try {
				...
				expect(error).toBeInstanceOf(IrcMessageLengthError);
				if (!(error instanceof IrcMessageLengthError)) throw error;
				expect(error.audience).toBe("direct");
				expect(error.limit).toBe(IRC_BODY_MAX_CHARS);
				expect(error.message).toContain("local:// or artifact:// path");
			}
			expect(bus.pollMessages("Recipient")).toEqual([]);
		});
	});

	it("persists and polls a direct message within the limit", () => {
		withExternalBus(bus => {
			bus.registerPeer({
	...
	it("refuses an oversized broadcast at its lower limit", () => {
		withExternalBus(bus => {
			bus.registerPeer({ sessionId: "recipient-session", name: "Recipient", cwd: "/tmp" });
			...
			).toThrow(`${IRC_BROADCAST_BODY_MAX_CHARS}-character limit`);
			expect(bus.pollMessages("Recipient")).toEqual([]);
		});
	});

	it("delivers a broadcast that fits the broadcast limit", () => {
		withExternalBus(bus => {
			bus.registerPeer({ sessionId: "recipient-session", name: "Recipient", cwd: "/tmp" });
			...
			expect(bus.pollMessages("Recipient").map(message => message.body)).toEqual([body]);
		});
	});
});
