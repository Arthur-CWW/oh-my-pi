...
describe("external IRC peer labels", () => {
	it("closes idempotently and rejects use after close before entering SQLite", () => {
		using tmp = TempDir.createSync("@omp-irc-bus-close-");
		const bus = new IrcExternalBus(`${tmp.path()}/irc-bus.sqlite`);
		bus.close();
		expect(() => bus.close()).not.toThrow();
		expect(() => bus.listPeers({ includeStale: true })).toThrow(IrcExternalBusClosedError);
	});
