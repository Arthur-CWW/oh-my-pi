...
export interface TerminalSessionWireServerOptions {
	...
	readonly ownerProof: (payload: JsonValue, hello: ClientHelloFrame | undefined) => JsonValue | Promise<JsonValue>;
...
class ServerConnection {
	...
	#receive(chunk: Buffer | string): void {
		...
		for (const frame of frames) {
			if (!this.#hello) {
				if (frame.kind === "request" && frame.operation === "ownerProof") {
	...
	async #dispatch(frame: RequestFrame): Promise<unknown> {
		if (frame.operation === "ownerProof") return this.#ownerProof(frame.payload, this.#hello);
