...
import type { IrcDeliveryReceipt, IrcDeliveryRecord, IrcMessageOrigin } from "./bus";
...
export class IrcExternalDeliveryError extends Error {
	readonly receipt: IrcDeliveryReceipt;

	constructor(receipt: IrcDeliveryReceipt) {
		super(receipt.error ?? "External IRC delivery refused");
		this.name = "IrcExternalDeliveryError";
		this.receipt = receipt;
	}
}

export class IrcExternalBusClosedError extends Error {
	constructor() {
		super("External IRC bus is closed");
		this.name = "IrcExternalBusClosedError";
	}
}
...
export class IrcExternalBus {
	...
	static global(): IrcExternalBus {
		if (!IrcExternalBus.#global || IrcExternalBus.#global.#closed) {
	...
	#assertOpen(): void {
		if (this.#closed) throw new IrcExternalBusClosedError();
	}
	...
	sendMessage(args: {
		...
		this.#assertOpen();
		const bodyLimit = args.broadcast ? IRC_BROADCAST_BODY_MAX_CHARS : IRC_BODY_MAX_CHARS;
		if (args.body.length > bodyLimit) {
			const kind = args.broadcast ? "broadcast" : "direct message";
			const error =
				`IRC ${kind} exceeds the ${bodyLimit}-character limit. ` +
				"Write the content to a file and send its local:// or artifact:// path instead.";
			logger.warn("IrcExternalBus: refused oversized message", {
				from: args.fromPeer,
				to: args.toPeer,
			...
			throw new IrcExternalDeliveryError({ to: args.toPeer, outcome: "failed", error });
		}
	...
	pollMessages(toPeer: string): IrcExternalMessage[] {
		...
		const rows = this.#db
			...
			.all({ $toPeer: toPeer });
		const messages: IrcExternalMessage[] = [];
		for (const row of rows) {
			if (row.body.length > IRC_BODY_MAX_CHARS) {
				this.markDelivered(row.id);
				logger.warn("IrcExternalBus: refused oversized inbound message", {
					from: row.from_peer,
					to: row.to_peer,
					bodyLength: row.body.length,
					bodyLimit: IRC_BODY_MAX_CHARS,
				});
				continue;
			}
