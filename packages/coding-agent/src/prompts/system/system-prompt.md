...
- `history://<agentId>`: agent transcript as concise markdown; `history://<session-id>/<agent-id>` and `history://<session-id>` are global read-only forms resolved via the sessions index (the short form stays session-local); bare `history://` lists agents
