...
- `agent(prompt, *, agent_type="implementer", model=None, label=None, schema=None)` — run ONE subagent; returns its final text, or the validated object when `schema` (a JSON Schema dict) is given. `agent_type` picks a discovered responsibility template ("implementer", "qa", "operator", "synthesizer", "explore", "reviewer", "designer", "plan", "librarian", "oracle", "quick_task"). Shared background goes in a `local://` file referenced from each prompt, not a parameter. `agent()` blocks until the subagent finishes; eval-spawned agents nest at most 3 deep.
