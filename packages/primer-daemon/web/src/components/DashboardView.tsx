...
/**
 * The original #/ dashboard, extracted so its hooks (useVimNav, usePolled, etc.)
 * only mount when this route is active. This avoids Rules-of-Hooks violations
 * in the parent router and prevents the dashboard's global keydown listener
 * from fighting with reader/review handlers.
 */
export function DashboardView(): React.JSX.Element {
  const [askHits, setAskHits] = useState<EvidenceHit[]>([])

  // Cards carry optimistic mutation, so DashboardView owns them.
  ...
  const nav = useVimNav({
    panels: [
      { id: "ask", count: askHits.length, onActivate: (i) => openUrl(askHits[i]?.url) },
      {
        ...
        onActivate: (i) => openUrl(progressList[i]?.refs.find((r) => /^https?:\/\//.test(r))),
      },
      {
        id: "cards",
        ...
        onActivate: (i) => {
          ...
          if (card)
            requestAnimationFrame(
              () => (document.querySelector(`[data-card-approve="${card.id}"]`) as HTMLElement | null)?.focus(),
            )
        },
  ...
  return (
    <>
      <main className="mx-auto w-full max-w-3xl px-4 py-5 min-[1200px]:max-w-6xl">
        <div className="flex flex-col gap-5 min-[1200px]:grid min-[1200px]:grid-cols-[minmax(0,1fr)_380px] min-[1200px]:items-start min-[1200px]:gap-5">
          <div className={columnClass}>
            <AskPanel
              active={nav.isActivePanel("ask")}
              isFocused={(i) => nav.isFocused("ask", i)}
              onFocus={(i) => nav.focus("ask", i)}
              onEvidence={setAskHits}
            />
            <ProgressFeed
      ...
      {activeProof !== null ? (
        <ProofViewer key={activeProof} name={activeProof} onClose={() => setActiveProof(null)} />
      ) : null}
