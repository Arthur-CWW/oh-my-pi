...
export async function getGoogleCookies(): Promise<{
	cookies: CookieMap;
	warnings: string[];
} | null> {
...
async function importSqlite(): Promise<typeof import("node:sqlite") | null> {
	...
	try {
		...
		sqliteImportError = err instanceof Error ? err.message : String(err);
		return null;
	} finally {
...
async function queryCookieRows(
	...
	for (const line of raw.split("\n")) {
		if (!line) continue;
		const [name = "", value = "", host_key = "", encryptedHex = ""] = line.split("\t");
		const row: Record<string, unknown> = { name, value, host_key };
		if (encryptedHex) {
			try {
