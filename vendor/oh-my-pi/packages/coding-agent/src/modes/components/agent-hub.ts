...
function modelAuthBadge(provider: string | undefined): string {
	return SUBSCRIPTION_MODEL_PROVIDERS.has(provider ?? "")
		? theme.fg("success", "[SUB]")
		: theme.fg("warning", "[API]");
}
...
function modelLane(resolvedModel: string, maxLabelWidth = MODEL_LANE_WIDTH): string {
	...
	const label = truncateToWidth(abbreviateResolvedModel(parts), maxLabelWidth);
	return `${modelAuthBadge(parts.provider)} ${label}`;
}

function modelHeaderLane(resolvedModel: string): string {
	const parts = parseResolvedModel(resolvedModel);
	const abbreviated = abbreviateResolvedModel(parts);
	const lane = `${modelAuthBadge(parts.provider)} ${truncateToWidth(abbreviated, MODEL_LANE_WIDTH)}`;
	if (parts.raw === abbreviated) return lane;
	return `${lane} ${theme.fg("dim", `(${truncateToWidth(parts.raw, HEADER_FULL_MODEL_WIDTH)})`)}`;
}
...
export class AgentHubOverlayComponent extends Container {
	...
	#renderRow(ref: AgentRef, selected: boolean, width: number): string {
		...
		if (observed?.progress?.resolvedModel) {
			parts.push(modelLane(observed.progress.resolvedModel));
		}
