...
import { getTab, releaseTab, releaseTabsOwnedBy } from "@oh-my-pi/pi-coding-agent/tools/browser/tab-supervisor";
...
describe("child browser tab ownership", () => {
	...
	for (const disposalPath of ["terminal", "abort"] as const) {
		it(`releases only the child's tab on ${disposalPath} disposal`, async () => {
			...
			// The browser tool registers tabs under their workstream-grouped
			// label; sessions here have no workstream, so group is "adhoc".
			const parentTabName = groupedBrowserTabName(FALLBACK_BROWSER_TAB_GROUP, parentName);
			const childTabName = groupedBrowserTabName(FALLBACK_BROWSER_TAB_GROUP, childName);
			let childDisposed = false;

			try {
				await parentTool.execute("parent-open", {
					action: "open",
					name: parentName,
					url: "about:blank",
					timeout: 15,
				...
				const parentTab = getTab(parentTabName);
				const childTab = getTab(childTabName);
				...
				await waitForPendingRun(childTabName);
				...
				expect(getTab(childTabName)).toBeUndefined();
				expect(await releaseTabsOwnedBy(childManager.getSessionId())).toBe(0);

				const survivingParentTab = getTab(parentTabName);
				...
				expect(getTab(parentTabName)).toBeUndefined();
				expect(parentTab?.browser.refCount).toBe(0);
			} finally {
				await releaseTab(childTabName).catch(() => undefined);
				await releaseTab(parentTabName).catch(() => undefined);
