...
import { EVAL_AGENT_MAX_DEPTH, runEvalAgent } from "../../src/eval/agent-bridge";
...
describe("runEvalAgent", () => {
	...
	it("returns a completed result even when the parent eval signal is already aborted", async () => {
		mockAgents();
		const runSpy = vi.spyOn(taskExecutor, "runSubprocess");
		runSpy.mockImplementationOnce(async options =>
			singleResult(options, {
		...
		const result = await runEvalAgent({ prompt: "hello" }, { session: makeSession(), signal: abortController.signal });
