#!/usr/bin/env python3
"""Merge a TikTok context.json (transcript + description) into a v2 decomposition.

Parses `2026-05-20_7642101474981367054.context.json`, where the transcript
covers the China CEO-drama ban and the description covers the South Korea
fertility / China comparison + overarching narrative, and emits the valid
compact JSON artifact described by docs/prompts/tiktok-video-decomposition-v2.md.
"""
import json
import sys
from pathlib import Path

CONTEXT_PATH = Path(__file__).resolve().parent.parent / "data/video-recreation/samuelszuchan/bootstrap-20260620/2026-05-20_7642101474981367054.context.json"
OUT_PATH = CONTEXT_PATH.with_suffix(".decomposition-v2.json")

# Allowed vocabularies (controlled by the spec).
...
def build_decomposition(ctx):
    duration = ctx["duration"] or 129.0
    transcript = ctx["transcript"]
    description = ctx["description"]

    # Scene blocks: merge transcript (China) + description (SK comparison / arc).
    scene_blocks = [
        {
            ...
            "visual_summary": "Full-frame text card punches the hook promise; accent red on 'banned'.",
        ...
        },
        {
            ...
            "narration_summary": "For a decade Chinese streaming was dominated by 'ba dao zong shi' — the overbearing CEO genre.",
            "visual_summary": "Genre title card with romanized + translated name; timeline bar 2013–2021 of genre dominance.",
            "key_claims": ["Genre called 'ba dao zong shi' (overbearing CEO).","Dominated Chinese streaming for ~a decade."],
            "proof_required": ["Genre title card with romanization + translation.","Decade-span timeline graphic."]
        },
        {
            ...
            "visual_summary": "Generated silhouette scene of the lobby drop; dossier card of the CEO (age 28, 15B yuan company, Maybach, Pudong penthouse).",
            "key_claims": ["CEO is 28 years old.","Company worth 15 billion yuan.","Drives a Maybach.","Pudong penthouse, glass-walled wine cellar."],
            "proof_required": ["CEO dossier counter (age, net worth).","Generated scene of penthouse/wine cellar (clean-room silhouette)."]
        },
        {
            "block_index": 3, "start_seconds": 52.0, "end_seconds": 72.0,
            "block_function": "mechanism",
            "dominant_a_roll_or_b_roll": "B-roll",
            "narration_summary": "The audience: women 18–35 in tier-2/3 cities, 4–12,000 yuan/month, many without local hukou (controls schools, healthcare, right to buy property); tier-1 home prices 15–25x median salary.",
            "visual_summary": "Map of China with tier-2/3 city pins; income counter 4–12k yuan; hukou icon stack; housing-affordability ratio bar.",
            "key_claims": ["Audience women 18–35 in tier-2/3 cities.","Income 4–12,000 yuan/month.","Many lack local hukou.","Tier-1 home prices 15–25x median salary."],
            "proof_required": ["China map with tier city pins.","Income counter (4–12k yuan).","Hukou iconography diagram.","Housing-to-income ratio graph."]
        },
        {
            "block_index": 4, "start_seconds": 72.0, "end_seconds": 88.0,
            "block_function": "mechanism",
            "dominant_a_roll_or_b_roll": "mixed",
            "narration_summary": "The female lead never works her way up — the genre told its audience your situation won't be resolved by your own action but by something that happens to you.",
            ...
            "key_claims": ["Female lead never starts a business or passes a difficult exam.","Genre message: resolution comes from an event, not action."],
            "proof_required": ["Loop diagram of the rescue mechanism.","Comparison node diagram (own-action vs rescue)."]
        },
        {
            ...
            "key_claims": ["2021 directives against money worship.","Luxury logos blurred; penthouse sets reshot austere.","By 2023 contemporary CEO drama effectively gone.","Replacements: rural revival, military romances, tang-dynasty palace intrigue."],
            "proof_required": ["Split-screen before/after of set treatment.","Replacement-genre icon grid.","Regulatory timeline."]
        },
        {
            "block_index": 6, "start_seconds": 106.0, "end_seconds": 122.0,
            "block_function": "consequence",
            "dominant_a_roll_or_b_roll": "mixed",
            "narration_summary": "South Korea never banned its CEO dramas (Boys Over Flowers, The Heirs, Crash Landing on You) yet fertility hit 0.72, Seoul 0.64, $270B over 16 years with negligible effect; demographics haven't slowed — whatever drives women out operates below drama regulation.",
            "visual_summary": "Korea vs China split screen; Korea fertility line graph to 0.72; $270B counter; K-drama title callouts.",
            "key_claims": ["SK fertility 0.72 in 2023; Seoul 0.64.","$270B+ spent over 16 years on pronatalist incentives.","Korea never banned CEO dramas (Boys Over Flowers, The Heirs, Crash Landing on You).","Korea demographic collapse hasn't slowed."],
            "proof_required": ["Korea fertility line graph with 0.72/0.64 marked.","$270B rolling counter.","K-drama title document callouts.","Korea/China comparison split screen."]
        },
        {
            "block_index": 7, "start_seconds": 122.0, "end_seconds": 129.0,
            "block_function": "synthesis",
            "dominant_a_roll_or_b_roll": "A-roll",
            "narration_summary": "Each ban is an admission of anxiety about which story is winning; China's official youth unemployment crossed 16% in March (suspected higher); adjusting fiction only adjusts what the economy feels like.",
            ...
            "key_claims": ["China official youth unemployment crossed 16% in March.","Real number suspected higher.","Bans are admissions of anxiety about which story is winning."],
            "proof_required": ["Youth-unemployment counter (16%).","Closing full-frame text card."]
        },
    ]

    # Shot beats: ~2–4s cadence across the 129s runtime.
    beat_specs = [
        (0.0, 3.5, "hook text card", False, "generated_scene", "push_in", "none", "full_frame_text_card", "'China banned Cinderella stories.'"),
        (3.5, 7.0, "hook promise sustained", True, "other", "static", "hard_cut", "ken_burns_stop", "'Here's why.'"),
        (7.0, 10.5, "genre name reveal", False, "document_callout", "push_in", "hard_cut", "reverse_swell", "'ba dao zong shi — overbearing CEO'"),
        (10.5, 14.0, "decade timeline", False, "graph", "pan_left", "match_cut", "none", "2013–2021 dominance bar"),
        (14.0, 18.0, "trope setup: lobby drop", False, "generated_scene", "push_in", "crossfade", "speed_ramp", "'A girl drops something in a lobby.'"),
        (18.0, 22.0, "CEO catches — silhouette closeup", False, "silhouette", "unexpected_closeup", "whip_pan", "unexpected_closeup", "'28-year-old billionaire catches it.'"),
        (22.0, 26.0, "CEO dossier card", False, "counter", "static", "hard_cut", "none", "age 28"),
        (26.0, 30.0, "15B yuan company metric", False, "counter", "push_in", "hard_cut", "ken_burns_stop", "15 billion yuan"),
        (30.0, 34.0, "Maybach / penthouse iconography", False, "iconography", "pan_right", "crossfade", "none", "Maybach icon"),
        (34.0, 38.0, "Pudong penthouse generated scene", False, "generated_scene", "push_in", "match_cut", "none", "glass-walled wine cellar"),
        (38.0, 42.0, "audience map: tier-2/3 cities", False, "map", "zoom_out", "whip_pan", "split_screen_comparison", "China map pins"),
        (42.0, 46.0, "audience demo: women 18–35", False, "diagram", "static", "hard_cut", "none", "18–35 cohort"),
        (46.0, 50.0, "income counter 4–12k yuan", False, "counter", "push_in", "hard_cut", "ken_burns_stop", "4–12,000 yuan/month"),
        (50.0, 54.0, "hukou mechanism diagram", False, "diagram", "push_in", "crossfade", "none", "hukou gates: schools/healthcare/property"),
        (54.0, 58.0, "housing affordability graph", False, "graph", "pan_right", "hard_cut", "ken_burns_stop", "15–25x median"),
        (58.0, 62.0, "female-lead never advances", False, "diagram", "static", "match_cut", "color_flash", "crossed-out 'own action'"),
        (62.0, 66.0, "rescue loop diagram", False, "diagram", "push_in", "crossfade", "none", "loop: event rescues you"),
        (66.0, 70.0, "genre message text card", False, "other", "static", "hard_cut", "full_frame_text_card", "'Resolved by something that happens to you.'"),
        (70.0, 74.0, "regulation pivot 2021", True, "document_callout", "push_in", "whip_pan", "sudden_silence_drop", "2021 directive"),
        (74.0, 78.0, "before/after set split screen", False, "generated_scene", "static", "match_cut", "split_screen_comparison", "ostentatious → austere"),
        (78.0, 82.0, "logo blur visual treatment", False, "generated_scene", "push_in", "hard_cut", "color_flash", "blurred luxury logo"),
        (82.0, 86.0, "timeline by 2023 gone", False, "graph", "pan_left", "hard_cut", "none", "2023 removal marker"),
        (86.0, 90.0, "replacement genre grid", False, "iconography", "static", "crossfade", "none", "rural / military / tang palace"),
        (90.0, 94.0, "referendum on official story", True, "diagram", "push_in", "hard_cut", "ken_burns_stop", "official-story nodes"),
        (94.0, 98.0, "youth unemployment counter 16%", False, "counter", "push_in", "hard_cut", "color_flash", "16% in March"),
        (98.0, 102.0, "real number suspected higher", False, "graph", "zoom_out", "crossfade", "unexpected_closeup", "uncertainty band"),
        (102.0, 106.0, "ban = anxiety admission", True, "other", "static", "hard_cut", "full_frame_text_card", "'which story is winning'"),
        (106.0, 110.0, "Korea vs China split screen", False, "map", "static", "whip_pan", "split_screen_comparison", "Korea | China"),
        (110.0, 114.0, "Korea fertility line graph", False, "graph", "pan_right", "hard_cut", "ken_burns_stop", "0.72 / 0.64 marks"),
        (114.0, 118.0, "$270B counter over 16y", False, "counter", "push_in", "hard_cut", "reverse_swell", "$270 billion"),
        (118.0, 122.0, "K-drama title callouts", False, "document_callout", "static", "crossfade", "none", "Boys Over Flowers / The Heirs / Crash Landing on You"),
        (122.0, 126.0, "below regulation diagram", False, "diagram", "zoom_out", "crossfade", "speed_ramp", "demographics below drama-regulation layer"),
        (126.0, 129.0, "closing text card synthesis", True, "other", "push_in", "dip_to_black", "full_frame_text_card", "'adjusting the fiction adjusts what the economy feels like.'"),
    ]

    shot_beats = []
    for i, (s, e, fn, a_roll_active, asset_cat, motion, t_in, interrupt, caption) in enumerate(beat_specs):
        beat = {
            "beat_index": i,
            "start_seconds": s,
            "end_seconds": e,
            ...
            "b_roll": {
                "active": not a_roll_active or True,
                "description": fn,
                "asset_category": asset_cat
            },
            "motion_cue": motion,
            "transition_in": t_in,
            "transition_out": "hard_cut" if i < len(beat_specs) - 1 else "dip_to_black",
            "caption_emphasis": caption,
            "_interrupt": interrupt  # consumed to build schedule below
        }
        shot_beats.append(beat)

    graphics_plan = {
        "maps_and_geography": [
            {"claim": "Audience concentrated in China tier-2/3 cities", "map_type": "city_map",
             "annotations": ["tier-2 cluster","tier-3 cluster","shadow hukou exclusion zones"], "style_notes": "flat outline, accent pins, animated highlight sweep"},
            {"claim": "Korea vs China regulatory divergence", "map_type": "country_outline",
             "annotations": ["South Korea","China","split-screen comparison line"], "style_notes": "two-panel outline to contrast drama policy"},
        ],
        "diagrams": [
            {"claim": "Hukou gates access to schools/healthcare/property", "diagram_type": "flow_chart",
             "elements": ["hukou gate","schools","healthcare","property right"], "style_notes": "gate icon blocking each branch"},
            {"claim": "Rescue loop: situation resolved by an event", "diagram_type": "loop",
             "elements": ["female lead","drop event","billionaire rescue","status quo restored"], "style_notes": "circular arrows, accent on 'no own-action' path"},
            {"claim": "Demographic drivers operate below drama regulation", "diagram_type": "node_network",
             "elements": ["drama regulation","housing prices","youth unemployment","fertility"], "style_notes": "drama-regulation node dimmed to show limited reach"},
        ],
        ...
        "document_callouts": [
            {"claim": "Genre name 'ba dao zong shi'", "document_type": "report_excerpt",
             "extracted_quote": "ba dao zong shi — the overbearing CEO", "visual_treatment": "romanization + translation, underline"},
            {"claim": "2021 regulatory directives against money worship", "document_type": "legal_text",
             "extracted_quote": "directives against what they considered money worship", "visual_treatment": "austere document, highlight on 'money worship'"},
            {"claim": "Korea never banned CEO dramas", "document_type": "news_headline",
             "extracted_quote": "Boys Over Flowers, The Heirs, Crash Landing on You still dominate streaming charts", "visual_treatment": "three title cards"},
        ],
        "iconography": [
    ...
    edit_fx_plan = {
        ...
        "pattern_interrupt_schedule": [
            ...
            for b in shot_beats if b.get("_interrupt") and b["_interrupt"] != "none"
        ],
    }

    caption_emphasis_plan = {
        "font_family_guess": "geometric sans (e.g. Inter / Söhne)",
        "text_color": "#F2F2F2",
        ...
        "key_phrases_to_emphasize": ["banned Cinderella stories", "overbearing CEO", "15 billion yuan", "hukou", "15 to 25 times median salary", "money worship", "0.72", "270 billion", "Boys Over Flowers", "adjusting the fiction adjusts what the economy feels like"],
        "caption_timing_rules": ["on-screen min 2.5s for numeric claims", "highlight accent appears 0.3s after caption", "text cards hold full-frame 1.5–2.5s"],
    }

    render_component_specs = [
        {"component_id": "TextCard", "component_type": "CaptionOverlay", "used_in_beats": [0,1,18,28,34], "props": {"text": "", "full_frame": True}, "remotion_equivalent": "<CaptionOverlay/>"},
        ...
        {"component_id": "DecadeTimeline", "component_type": "Graph", "used_in_beats": [3,22], "props": {"graph_type": "line", "x": "year", "y": "genre share"}, "remotion_equivalent": "<Graph/>"},
        ...
        {"component_id": "RescueLoop", "component_type": "NodeDiagram", "used_in_beats": [16,17], "props": {"diagram_type": "loop"}, "remotion_equivalent": "<NodeDiagram/>"},
        {"component_id": "DirectiveDoc", "component_type": "DocumentCallout", "used_in_beats": [19], "props": {"quote": "money worship", "treatment": "austere"}, "remotion_equivalent": "<DocumentCallout/>"},
        {"component_id": "SplitScreen", "component_type": "BackgroundGrid", "used_in_beats": [20,28], "props": {"panels": 2}, "remotion_equivalent": "<BackgroundGrid/>"},
        {"component_id": "ReplacementGrid", "component_type": "DocumentCallout", "used_in_beats": [23], "props": {"genres": ["rural revival","military romance","tang palace"]}, "remotion_equivalent": "<DocumentCallout/>"},
        {"component_id": "UnemploymentCounter", "component_type": "MetricCounter", "used_in_beats": [25], "props": {"value": 16, "unit": "percent"}, "remotion_equivalent": "<MetricCounter/>"},
        {"component_id": "KoreaFertilityGraph", "component_type": "Graph", "used_in_beats": [29], "props": {"graph_type": "line", "marks": [0.72, 0.64]}, "remotion_equivalent": "<Graph/>"},
        {"component_id": "SKSpendingCounter", "component_type": "MetricCounter", "used_in_beats": [30], "props": {"value": 270, "unit": "billion USD/16y"}, "remotion_equivalent": "<MetricCounter/>"},
        {"component_id": "KDramaCallouts", "component_type": "DocumentCallout", "used_in_beats": [31], "props": {"titles": ["Boys Over Flowers","The Heirs","Crash Landing on You"]}, "remotion_equivalent": "<DocumentCallout/>"},
    ]
    ...
    uncertainties = [
        ...
        "Official youth unemployment 'crossed 16% in March' — description cites 21.3% by June 2023 (suspended); reconcile.",
    ]

    decomposition = {
        "source_video_id": ctx["id"],
        "transcript": {
            "verbatim_or_vtt_cleaned": transcript,
            "notable_asr_uncertainties": [
                "'ba dao zong shi' romanization drift",
                "'15 billion yen' likely 'yuan'",
                "'SA Song Don' style ASR slips in sibling transcript (not this one) — keep guard",
        ...
        "shot_beats": [{k: v for k, v in b.items() if k != "_interrupt"} for b in shot_beats],
        "graphics_plan": graphics_plan,
        "edit_fx_plan": edit_fx_plan,
...
def validate(decomposition, shot_beats_with_interrupts):
    # All controlled-vocab fields.
    for b in shot_beats_with_interrupts:
        ...
        assert b["b_roll"]["asset_category"] in ASSET_CATEGORIES, f"bad asset_category: {b}"
    ...
    # >=3 pipeline failures.
    assert len(decomposition["pipeline_failures_to_watch"]) >= 3
    # Every numeric claim has a counter or graph.
    # Spot-check key numbers appear in counters_or_metrics.
...
def main():
    ctx = load_context()
    decomposition = build_decomposition(ctx)
    # Rebuild beats-with-interrupt for validation (the _interrupt was stripped for output).
    # Instead, validate against edit_fx_plan schedule + raw specs would need rework; do simpler checks.
    # Mail parse original shot_beats still carry _interrupt? They were stripped. Validate vocab independently.
    # (Re-validate using the schedule and scene blocks.)
    for b in decomposition["shot_beats"]:
        assert b["transition_in"] in TRANSITIONS
        assert b["transition_out"] in TRANSITIONS
        assert b["b_roll"]["asset_category"] in ASSET_CATEGORIES
    for m in decomposition["edit_fx_plan"]["music_categories"]:
        assert m["category"] in MUSIC_CATS
    for p in decomposition["edit_fx_plan"]["pattern_interrupt_schedule"]:
        assert p["device"] in INTERRUPTS
    assert len(decomposition["edit_fx_plan"]["music_categories"]) == len(decomposition["scene_blocks"])
    distinct = {p["device"] for p in decomposition["edit_fx_plan"]["pattern_interrupt_schedule"]}
    assert len(distinct) >= 4, f"need >=4 interrupt types, got {len(distinct)}"
    assert len(decomposition["pipeline_failures_to_watch"]) >= 3
    counter_text = json.dumps(decomposition["graphics_plan"]["counters_and_metrics"])
    for needle in ["15", "12000", "0.72", "270", "16"]:
        assert needle in counter_text, f"missing numeric proof for {needle}"
    ...
    print(f"bytes={len(compact)} beats={len(decomposition['shot_beats'])} scenes={len(decomposition['scene_blocks'])} interrupts={len(distinct)}")
