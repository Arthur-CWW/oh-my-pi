...
async function main() {
	console.log("\npi-web-access Chrome cookies e2e smoke test (Effect)\n");

	const exit = await Effect.runPromiseExit(readChromeCookiesEffect());
	if (exit._tag === "Failure") {
		fail("Cookie extraction failed in Effect layer");
	}
	...
	ok(`Extracted ${cookieNames.length} Google cookies from ${exit.value.source} (required Gemini cookies present)`);
	...
	console.log("\n🎉 E2E passed: Effect cookie extraction -> Gemini Web cookie-auth path works\n");
}
