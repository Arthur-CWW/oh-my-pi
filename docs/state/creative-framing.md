...
- **Nietzsche for the register, Soros for the mechanism.** Nietzsche is the prose standard — *die fröhliche Wissenschaft*, the joyful science: ecstatic, aphoristic, affirming; hope-core philosophy avant la lettre, and the register Xanadu docs should aspire to. His three metamorphoses end in the child: creation as play, "a sacred Yes" — the playground's patron concept. Soros matters for the *idea*, not the prose: **reflexivity** — beliefs alter the fundamentals they describe — is hyperstition in respectable clothes, and his practice (when a strategy works, scale it hard before the edge closes) is already the factory's scaling rule. (Arthur's Nietzsche reading notes: `~/vault/nietzsche`.)
