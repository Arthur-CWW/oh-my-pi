...
- Jimeng ControlNet pose preview is now a usable no-spend UGC primitive: `jimeng-browser-proxy controlnet-preview --image <path> --control pose` uploads to ImageX when needed, calls `/mweb/v1/blend_preview` plus `/mweb/v1/pose_detect`, saves a local preview skeleton PNG, and records the frontend save-param patch for later pose-transfer/reference payloads. Depth/canny share the implemented flag path but still need representative live smokes; style/reference/object segmentation remain next mapping targets.
...
- Keep centralized metadata for prompts, providers, tags, vibes, intended use, and provenance.
- Archive public inspiration respectfully; avoid Twitter/X bans and do not capture likes/private/locked content.
...
- For anime/manga-style lanes, use user-provided/licensed assets or original generated assets; do not depend on scanlation/piracy scraping.
