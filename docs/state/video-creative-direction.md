...
- Jimeng saved subject/persona listing is now a usable no-spend primitive: `jimeng-browser-proxy subjects --limit 20` calls `/mweb/v1/dreamina_subject/get` and records existing subject ids/names/image refs/voice refs when present. The current account returned zero saved subjects, so subject/persona create/update/generate_voice and custom voice clone remain capture targets.
