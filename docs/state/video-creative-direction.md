...
- Jimeng ControlNet preview is now a usable no-spend UGC primitive: `jimeng-browser-proxy controlnet-preview --image <path> --control pose|depth|canny` uploads to ImageX when needed, calls `/mweb/v1/blend_preview`, saves a local preview PNG, and records the frontend save-param patch for later pose/depth/outline reference payloads. Pose also calls `/mweb/v1/pose_detect`. Style/reference/object segmentation remain next mapping targets.
