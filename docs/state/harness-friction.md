...
| --- | --- | --- | --- |
| 2026-07-03 | Presence `waiting_input` covers post-turn prompt only; mid-turn `ask`-tool waits still report `working` on the irc bus. Wire src/tools/ask.ts to updatePeerState when presence needs finer grain. | vendor/oh-my-pi/packages/coding-agent/src/session/agent-session.ts:2075 | low |
