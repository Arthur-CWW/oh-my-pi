...
18. Export reference-profile mechanics for UGC Studio as a typed package handoff, not a second archive store: `ReferenceProfileArchiveExport` should compose existing `ArchiveUser`, `ArchiveTweet`, `ArchiveMedia`, and `ArchiveTweetTimelineProvenance` evidence with pose/timing, gesture, shot, hook, caption, CTA, posting-strategy, preserve/swap, and blocked-use fields.
...
- `archive_jobs` candidate queue rows for `@pleometric` and artifact-extraction candidates carry `source_lane`, `target_type`, `target_value`, `status`, `priority`, `attempts`, timestamps, and provenance JSON (`promotionReason`, `artifactExtractionCandidateId`, source signal/bookmark/history id, or blocker reason). `pending` means eligible for a later explicit bounded public/archive capture; missing approval or source metadata must stay outside worker-claimable rows or remain visible as blocker provenance until resolved; `skipped` keeps a policy/private/deleted/rate-limit reason. Queue display and fixture import do not perform live scraping.
...
- Reference-profile exports do not require a V1 SQLite table. They are derived package API objects composed from `archive_users`, `archive_tweets`, `archive_media`, `tweet_timeline_provenance`, provenance JSON, and optional local annotations; UGC persistence should store the derived JSON artifact under its workspace `reference-archives` path with source row ids, not duplicate the Twitter archive schema.
...
| `reference-profile-export` | Typed handoff for UGC reference-profile archive/decomposition built from stored Twitter archive rows. | `packages/twitter-archive/src/index.ts`, `docs/plans/ugc-studio-workstreams.md`, `docs/plans/twitter-archive-goal.md` | Local proof uses an existing SQLite/dev-API state, constructs `ReferenceProfileArchiveExport`, verifies every mechanic evidence tweet/media id resolves to stored archive rows, and confirms the payload stores mechanics plus ids/provenance rather than raw media/transcript copies. |
...
- `@pleometric` and artifact-extraction candidates can be proven without live scraping by importing a tiny local fixture into `archive_jobs`, then inspecting the queued `profile` row, status, priority, and provenance; public capture remains a separate explicit bounded command.
...
- Reference-profile export is available as `ReferenceProfileArchiveExport`; a local fixture/proof can derive pose/timing/hook/caption/CTA mechanics from stored archive rows and keep source evidence as tweet/media/provenance ids.
