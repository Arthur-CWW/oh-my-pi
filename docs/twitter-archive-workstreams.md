...
- The bookmark DevTools extension is allowed to observe X/Twitter network responses only while the user is logged in, has DevTools open, and controls which page is open and scrolled. It may POST captured response data to the localhost dev server only; it must not save cookies, bearer tokens, CSRF tokens, or replay private API requests.
...
15. Ingest supervised bookmark/network captures from `browser-extensions/extensions/x-bookmark-sync-devtools` through the local dev server, defaulting to `http://127.0.0.1:3420`; the extension is only a capture client and the server owns normalization, SQLite persistence, JSONL logging, and Markdown export.
...
- The dev UI local notes/tags/marks/attributes and the X bookmark DevTools extension are two inputs to the same local archive loop. Both write through the localhost Bun server into SQLite, append JSONL events where useful, and generate derived Markdown; neither path writes to Twitter/X.
...
- The extension default target should be the running dev server at `http://127.0.0.1:3420`, not the old unused ingest port. The extension remains a capture client: DevTools observes responses while the user controls X, then the server performs validation, persistence, queueing, and Markdown export.
...
- Browser bookmark imports are documented as a local file/SQLite candidate-target lane, while authenticated X bookmark contents are handled only by the explicit DevTools extension/export workflow.
...
- Bookmark DevTools sync captures only supervised network responses while the user is logged in and controlling X, posts to localhost, saves no cookies/tokens, performs no mutations, and results in local SQLite/Markdown artifacts only.
