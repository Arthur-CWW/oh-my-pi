...
- Lip-sync/digital-human contract inference now consumes the existing image/avatar lip-sync, VOD lip-sync, pre-process, and pre-process-query dry-run proof bundles. The proof run under `data/jimeng-lab/proof-20260612-lip-sync-human-contract-infer/` generated scaffolds for `/mweb/v1/aigc_draft/generate`, `/mweb/v1/video_generate/pre_process`, and `/mweb/v1/video_generate/mget_pre_process_result`, with useful CLI flag suggestions for `--imageUri`, `--videoVid`, `--videoUri`, `--text`, `--speed`, `--mode`, `--voice-id`, and `--submitId`. Browser-backed image/avatar `cdp-ui` submit wiring now targets the real digital-human workbench `?type=digitalHuman&workspace=undefined`; local role upload, voice selection, scene-2/scene-7 pre-process, and `tts_generate` all fire in the live UI and are saved under `data/jimeng-lab/packet-20260614-digitalhuman-ui-probe/`. The strongest current blocker is now provider-gated: scene `2` still returns empty `image_create_avatar_info.resource_id_std` / `resource_id_loopy` plus `fail_code="1152"` for the local upload, for all three saved subject avatars (`data/jimeng-lab/packet-20260614-all-subject-avatar-preprocess/normalized/summary.json`), and for the scene-7 matched provider image (`data/jimeng-lab/packet-20260614-matched-avatar-preprocess/normalized/summary.json`). Voice recommendation and `tts_generate` succeed, but `/mweb/v1/aigc_draft/generate` never fires because submit never enables. Treat `lip-sync-human` as blocked until a new role/avatar source or frontend-only role flow yields non-empty scene-2 resource ids.
