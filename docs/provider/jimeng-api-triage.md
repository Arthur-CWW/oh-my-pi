...
- `generation-contract` hand-tightens the inferred `/mweb/v1/aigc_draft/generate` proof shape into a typed submit/poll/artifact summary contract. The first replay over the 2026-06-12 live matrix wrote `generation-contract-summary.json` and Markdown under `data/jimeng-lab/proof-20260612-live-generation-matrix/generation-contract/normalized/generation-contract/` and validated 4 saved video generation proofs with 0 skipped candidates. `executeJimengVideoDirectSubmit` and `executeJimengText2ImageSubmit` add no-live-spend direct submit helpers around the existing plan builders, covered by mock-fetch tests in `docs/qa/jimeng-generation-direct-submit-client-20260613.md`; fresh 2026-06-13 UI capture proved stable text2image request-body parity, `cdp-fetch` still returned `ret=3018`, and the browser-backed `jimeng-browser-proxy text2image --transport cdp-ui` path now live-proves text2image submit/poll/download under `data/jimeng-lab/proof-20260613-goal-text2image-cdp-ui/`. The remaining text2image gap is endpoint-level direct signer/a_bogus recovery, not useful-workflow parity.
...
- Lip-sync/digital-human contract inference now consumes the existing image/avatar lip-sync, VOD lip-sync, pre-process, and pre-process-query dry-run proof bundles. The proof run under `data/jimeng-lab/proof-20260612-lip-sync-human-contract-infer/` generated scaffolds for `/mweb/v1/aigc_draft/generate`, `/mweb/v1/video_generate/pre_process`, and `/mweb/v1/video_generate/mget_pre_process_result`, with useful CLI flag suggestions for `--imageUri`, `--videoVid`, `--videoUri`, `--text`, `--speed`, `--mode`, `--voice-id`, and `--submitId`. New 2026-06-14 headed background-browser evidence moved `lip-sync-human` back to active implementation: on the real digital-human workbench `?type=digitalHuman&workspace=undefined`, local role upload + voice selection + real script entry eventually enabled submit, `/mweb/v1/aigc_draft/generate` returned `ret=0`, and replaying that exact captured request directly through `jimeng-browser-proxy endpoint-probe` also returned `ret=0` without browser UI. Proof roots: `data/jimeng-lab/packet-20260614-headed-digitalhuman-fullprep/`, `data/jimeng-lab/packet-20260614-headed-digitalhuman-submit/`, and `data/jimeng-lab/packet-20260614-headed-digitalhuman-direct-replay/`. The highest-value next slice is now contract/payload parity: promote the headed generate body/query into the clean-room lip-sync image submit path so direct submit/poll/download works without browser dependence.
- `R1 /mweb/v1/mget_story` is now best treated as blocked rather than partially complete for goal-completion purposes. The `story-records` typed client and replay tests exist, but a 2026-06-11 sweep over three existing followed profiles (`PUAI`, `小波登`, `就扶墙老师`) returned `ret=0` with `story_count=0`; promotion still needs a public profile or UI capture with a non-empty story list and real `story_id_list` values. Treat this as a low-priority supporting-read blocker, not a reason to prioritize story/archive reads over generation, persona/voice, lip-sync, reference controls, or template mining.
