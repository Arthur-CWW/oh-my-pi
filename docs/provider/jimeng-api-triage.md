...
- Lip-sync/digital-human contract inference now consumes the existing image/avatar lip-sync, VOD lip-sync, pre-process, and pre-process-query dry-run proof bundles. The proof run under `data/jimeng-lab/proof-20260612-lip-sync-human-contract-infer/` generated scaffolds for `/mweb/v1/aigc_draft/generate`, `/mweb/v1/video_generate/pre_process`, and `/mweb/v1/video_generate/mget_pre_process_result`, with useful CLI flag suggestions for `--imageUri`, `--videoVid`, `--videoUri`, `--text`, `--speed`, `--mode`, `--voice-id`, and `--submitId`. Browser-backed image/avatar `cdp-ui` submit wiring now exists and focused tests/typecheck pass, but the current `https://jimeng.jianying.com/ai-tool/generate/?type=lip_sync` route still renders a generic generation composer. Selecting a reusable 2048x2048 recent image only exposes generic image-generation controls (`图片5.0 Lite`, `1:1`, `2K`, `时间`, `生成模式`, `操作类型`), and the first live lip-sync attempt failed with `JIMENG_LIP_SYNC_VOICE_OPTION_MISSING`. Remaining `lip-sync-human` work is therefore blocked on capturing or locating the real lip-sync/digital-human workbench state and stable voice-picker DOM, not on polling/download plumbing.
