...
- Persona/voice contract inference now consumes dry-run request plans that expose `endpoint_sequence`. The proof run under `data/jimeng-lab/proof-20260612-persona-voice-contract-infer/` generated scaffolds for subject voice generation, voice clone submit/query/update/delete, and single audio/video mix, with useful CLI flag suggestions for `--imageUri`, `--audioVid`, `--name`, `--taskIds`, `--voice-id`, and `--videoItemId`. Subject voice and voice clone submit/query/update/delete are now classified as partial because typed helpers and replay/mocked client tests exist; the remaining `persona-voice` gap is live capture/approval for provider task creation and mutation, plus mix-audio typed-service promotion.
