...
1. `lip-sync-human`: avatar or VOD lip-sync submit/poll/download, pre-process result promotion, and talking-head examples.
2. `supporting-reads`: only when they directly unblock the remaining lip-sync-human capture/proof work or the direct signer follow-up for `/mweb/v1/aigc_draft/generate`.
