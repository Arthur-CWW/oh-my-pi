...
- Lip-sync/digital-human contract inference now consumes the existing image/avatar lip-sync, VOD lip-sync, pre-process, and pre-process-query dry-run proof bundles. The proof run under `data/jimeng-lab/proof-20260612-lip-sync-human-contract-infer/` generated scaffolds for `/mweb/v1/aigc_draft/generate`, `/mweb/v1/video_generate/pre_process`, and `/mweb/v1/video_generate/mget_pre_process_result`, with useful CLI flag suggestions for `--imageUri`, `--videoVid`, `--videoUri`, `--text`, `--speed`, `--mode`, `--voice-id`, and `--submitId`. The current 2026-06-30 background CDP pass reaches the real digital-human workbench `?type=digitalHuman&workspace=undefined`, opens the voice panel, selects `直爽女大`, and fills TipTap `说话内容` plus `动作描述` in no-submit preflight. Proof note: `docs/qa/jimeng-lip-sync-digitalhuman-passive-capture-20260630.md`; ignored proof root: `data/jimeng-lab/packet-lip-sync-human-20260630/`. The highest-value next slice is one approved browser-backed image/avatar upload plus submit/poll/download proof, then contract promotion from the observed request/response.
