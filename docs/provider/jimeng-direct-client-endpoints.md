...
Latest proof found 247 static frontend/API resources, included 200 non-implemented resources, skipped 29 implemented endpoints, and counted 61 high-value gaps after the CapCut collection/row/detail endpoints were promoted. Known status counts are `unknown=193`, `partial=4`, `implemented=29`, `dry_run_only=4`, `blocked=14`, `captured_only=2`, and `cataloged_only=1`. The remaining CapCut template search/batch/preset endpoints are explicitly blocked with replay evidence and recommended action `capture_exact_payload`. Seven video-generation helper endpoints are also explicitly blocked pending exact UI payload capture: `/mweb/v1/video_generate/get_switch_model_queue_info`, `/mweb/v1/video_generate/pre_process`, `/mweb/v1/video_generate/mget_pre_process_result`, `/mweb/v1/video_generate/face_auth/skip`, `/mweb/v1/video_generate/face_auth/skip/query`, `/mweb/v1/aigc_draft/cancel_generate`, and `/mweb/v1/aigc_draft/generate_accelerate`. Normalized proof files contain no credential markers.
