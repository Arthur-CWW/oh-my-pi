...
- No-generation/no-spend read path for the canvas workspace project list, one project detail lookup, and custom canvas ratio presets.
...
```txt
endpoints=projects,detail,ratios
project_count=1
detail_mode=1
ratio_count=0
skipped=0
...
Normalized proof hashes creator user ids, summarizes draft JSON by counts/hash, and omits raw draft JSON, cookies, and signed media URLs.
...
- infinite canvas: `/mweb/v1/infinite_canvas/list_project`, `/mweb/v1/infinite_canvas/project_detail`, and `/mweb/v1/infinite_canvas/v1/get_canvas_custom_ratio` are implemented as no-spend `infinite-canvas`; create/update/delete/conversation/changeset/snapshot paths remain capture or disposable-fixture targets
...
| `infinite-canvas` | implemented in `jimeng-browser-proxy` | No-spend schema-backed `/mweb/v1/infinite_canvas/list_project`, `/mweb/v1/infinite_canvas/project_detail`, and `/mweb/v1/infinite_canvas/v1/get_canvas_custom_ratio` metadata reads. Latest proof returned one canvas project, one detail record, and zero custom ratios. |
