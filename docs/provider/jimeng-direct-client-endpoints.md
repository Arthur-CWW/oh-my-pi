...
- Current local live replay blocker: terminal DNS resolution for `jimeng.jianying.com` is unavailable (`scutil --dns` returned no DNS configuration), and known-good read commands also fail through the normal Bun fetch path. Public DNS still resolves the host, so this is local resolver state rather than endpoint evidence.
...
| `workspace-context` | implemented in `jimeng-browser-proxy` | Schema-backed no-spend `/mweb/v1/workspace/list` and `/mweb/v1/workspace/get_by_ids` workspace metadata reads. Static/dry-run proved; live replay pending local DNS resolver recovery. |
