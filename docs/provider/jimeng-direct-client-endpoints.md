...
- reference-profile research: `/mweb/v1/get_user_info`, `/mweb/v1/get_homepage`, `/mweb/v1/get_favorite_list`, `/mweb/v1/get_user_story_list`, `/mweb/v1/get_follow_list`, and `/mweb/v1/get_item_info` are implemented as no-spend `profile-research`; public profile reads use `sec_uid`, published work detail uses `published_item_id`, and follow/follower lists are current-account scoped. Batch `/mweb/v1/mget_item_info` is blocked pending exact payload recovery; safe guessed id-list variants returned `ret=1000 invalid parameter`
...
Current 2026-06-11 proof found 251 static frontend/API resources and counts `unknown=118`, `partial=4`, `implemented=51`, `dry_run_only=4`, `blocked=71`, `captured_only=2`, and `cataloged_only=1`. `profile-research` promoted six public/current-account social-reference reads: public profile, homepage, favorites, stories, current-account following/followers, and published item detail. The normalized proofs preserve prompts, model keys, first-frame provider URIs, dimensions, duration, story cover metadata, and engagement counters without signed URL values. Proof is under `data/jimeng-lab/proof-20260611-profile-research-cli-{dry-run,live}/`, `data/jimeng-lab/proof-20260611-profile-research-stories-{dryrun,live}/`, and `data/jimeng-lab/proof-20260611-static-inventory-profile-stories/`. Batch `/mweb/v1/mget_item_info` remains blocked after `ret=1000` no-spend probes under `data/jimeng-lab/proof-20260611-probe-mget-item-info/`.
