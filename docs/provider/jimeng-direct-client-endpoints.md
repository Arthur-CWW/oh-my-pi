...
Current 2026-06-11 proof found 251 static frontend/API resources and counts `unknown=118`, `partial=4`, `implemented=52`, `dry_run_only=4`, `blocked=70`, `captured_only=2`, and `cataloged_only=1`. `profile-research` now covers seven public/current-account social-reference reads: public profile, homepage, favorites, stories, current-account following/followers, single published item detail, and batch published item detail. The normalized proofs preserve prompts, model keys, first-frame provider URIs, dimensions, duration, story cover metadata, and engagement counters without signed URL values. Proof is under `data/jimeng-lab/proof-20260611-profile-research-cli-{dry-run,live}/`, `data/jimeng-lab/proof-20260611-profile-research-stories-{dryrun,live}/`, `data/jimeng-lab/proof-20260611-profile-research-batch-items-{dryrun,live}/`, and `data/jimeng-lab/proof-20260611-static-inventory-profile-batch-items/`. The superseded `ret=1000` proof at `data/jimeng-lab/proof-20260611-probe-mget-item-info/` used guessed field names; the successful probe at `data/jimeng-lab/proof-20260611-probe-mget-item-info-item-id-list/` recovered the frontend wire field `item_id_list`.
