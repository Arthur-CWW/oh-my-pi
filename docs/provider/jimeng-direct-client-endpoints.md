...
concurrency=64 requests=128 completed=128 stopped=false http=200x128 ret=0x128 p95=458ms
concurrency=96 requests=192 completed=192 stopped=false http=200x192 ret=0x192 p95=523ms
...
concurrency=2048 requests=2048 completed=2048 stopped=false http=200x2048 ret=0x2048 p50=10283ms p95=15822ms max=28013ms
```

This means no rate limit was observed up to concurrency `2048` on that read-only config endpoint. The `2048` tier used a direct IP/SNI transport override because the local macOS resolver temporarily had no DNS configuration. Tail latency becomes poor at the highest tiers, and this does **not** establish a safe limit for paid generation, upload, mutation, or polling endpoints.

### 6.0.1.1) Workspace context reads

- Implemented as `jimeng-browser-proxy workspace-context`.
- No-spend read path for logged-in Jimeng workspace/project metadata:
  - `/mweb/v1/workspace/list`
  - `/mweb/v1/workspace/get_by_ids`
- Static frontend evidence shows the service builds list bodies from `{ offset, limit }`, then looks up metadata by `{ workspace_ids: [...] }`.
- Useful flags: `--endpoints list,get-by-ids`, `--limit`, `--offset`, and `--workspaceIds`.
- Proof bundles:
  - static locate: `data/jimeng-lab/proof-20260610-static-locate-workspace-context/`
  - dry-run: `data/jimeng-lab/proof-20260610-workspace-context-dry-run/`
  - live no-spend: `data/jimeng-lab/proof-20260610-workspace-context-live/`
  - refreshed inventory: `data/jimeng-lab/proof-20260610-static-inventory-workspace-context/`
- Latest live proof returned `2` listed workspaces and `1` by-id workspace with `ret=0` on both endpoints. The schema accepts string or numeric workspace/user ids because the default workspace returned numeric `workspace_id=0`; normalized proof output hashes user/workspace-id request values and omits signed cover URLs.
...
### 6.0.4) Signed commerce benefit metadata

- Implemented as `jimeng-browser-proxy commerce-benefits`.
- Direct endpoints:
  - `POST https://jimeng.jianying.com/commerce/v3/resource/benefit_metadata`
  - `POST https://jimeng.jianying.com/commerce/v3/benefits/batch_get_user_benefit`
- No generation spend. This reads benefit/quota metadata and the current user's benefit rows for spend gating and model-option cost mapping.
- Uses the same frontend-compatible commerce sign headers as `account-credit`.
- Static frontend evidence in `CommercialBenefitDataService` showed the service-layer request is `queryList`, but the network serializer converts it to snake_case. The proved wire body is:
...
- The earlier camelCase probe returned `ret=1000`, `errmsg=invalid param`; the snake_case body returned `ret=0` on both endpoints.
- Normalized output includes resource ids, benefit types, units, use modes, pay modes, roles, quotas, `total_credits`, and `enable_preview`.
...
### 6.0.5) Infinite canvas project metadata
...
- Frontend bundle evidence:
  - `getCanvasProjectList` posts `/mweb/v1/infinite_canvas/list_project` with frontend object fields `cursor`, `limit`, `imageInfo`, and `onlyFavorite`.
  - `getProjectDetail` frontend callers pass `{projectId, option.needDraftResource}`, but the observed wire contract is snake-cased: `project_id` and `option.need_draft_resource`.
  - `getCustomRatios` frontend callers pass `{userId}`, but the observed wire contract is `user_id`.
  - `getConversationList` posts `/mweb/v1/infinite_canvas/get_conversation_list`; safe probes proved `project_id` is required and `offset`/`count` are accepted.

Live no-spend proof on 2026-06-10:
...
```txt
endpoints=projects,detail,ratios,conversations
project_count=1
detail_mode=1
ratio_count=0
...
Normalized proof hashes creator user ids, summarizes draft JSON by counts/hash, and omits raw draft JSON, cookies, and signed media URLs. `fetch_conversation` remains blocked until a non-empty conversation list returns a real `conversation_id`; create/update/delete/conversation/changeset/custom-ratio update paths are blocked on exact UI capture or disposable fixture approval.
...
- template/research mining: `/mweb/search/v1/sug`, `/mweb/search/v1/guess`, `/mweb/search/v1/search`, `/mweb/v1/feed`, `/mweb/v1/get_explore`, `/mweb/v1/feed_short_video`, `/lv/v1/cc_web/plane/get_categories`, public CapCut `bee_prod` metadata JSON, `/lv/v1/cc_web/replicate/search_templates`, `/lv/v1/cc_web/plane/*`; `research-keywords` implements no-spend suggestions and guesses, full Jimeng search remains partial pending its response transform, direct `/mweb/v1/get_explore` support is implemented for both templates and short-video examples, `/mweb/v1/feed_short_video` is implemented as `overseas-short-videos`, CapCut category catalog is implemented as `capcut-categories`, CapCut collection/row/detail browsing is implemented as `capcut-collections`, `capcut-collection-templates`, and `capcut-template-detail`, and public CapCut ratio/scene metadata is implemented as `capcut-template-metadata`; CapCut search, batch, and preset payloads still need real UI capture
...
- Status:
  - implemented as a session-free no-spend probe tool for the remaining CapCut row/search endpoints and an explicit allowlist of read-oriented LV editor endpoints
  - guarded to signed read-oriented CapCut/LV endpoints; `edit-api-sg.capcut.com` handles template/editor reads and `feed-api-sg.capcut.com` handles `/lv/v2/cc_web_task/*` task reads
  - rejects known mutating paths such as `/lv/v1/cc_web/plane/del_presets_template`
...
| `rate-probe` | implemented in `jimeng-browser-proxy` | Bounded concurrency/rate probe for no-spend read/config endpoints with stop-on-429/auth/risk behavior, latency percentiles, status/ret counts, and hash-only response evidence. Latest `/mweb/v1/get_common_config` sweep found no read/config limit through concurrency `2048`, though latency degraded sharply. |
...
| `workspace-context` | implemented in `jimeng-browser-proxy` | Effect Schema-backed no-spend `/mweb/v1/workspace/list` and `/mweb/v1/workspace/get_by_ids` workspace metadata reads. Latest live proof returned 2 listed workspaces and 1 by-id workspace. |
...
Latest proof found 251 static frontend/API resources, included 120 resources at the current review limit, skipped 44 implemented endpoints, and counted 61 high-value gaps after the Jimeng research keyword, CapCut collection/row/detail, LV editor catalog, infinite-canvas read, account credit, commerce-benefit, and workspace-context endpoints were promoted. Known status counts are `unknown=125`, `partial=5`, `implemented=44`, `dry_run_only=4`, `blocked=70`, `captured_only=2`, and `cataloged_only=1`. The `research-keywords` command live-proves `/mweb/search/v1/sug` and `/mweb/search/v1/guess`; latest proof returned 30 normalized terms across 5 successful sequential reads and skipped unsupported asset suggestions. `/mweb/search/v1/search` remains partial pending its frontend response transform, while `/mweb/search/v1/fetch_debug/search` is blocked as a non-production wrapper. The `workspace-context` command live-proves `/mweb/v1/workspace/list` and `/mweb/v1/workspace/get_by_ids`; latest proof returned 2 listed workspaces and 1 by-id workspace. The `commerce-benefits` command live-proves `/commerce/v3/resource/benefit_metadata` and `/commerce/v3/benefits/batch_get_user_benefit`; latest proof returned 12 metadata rows and 140 user benefit rows. The `infinite-canvas` command live-proves four no-spend Jimeng canvas read endpoints: `/mweb/v1/infinite_canvas/list_project`, `/mweb/v1/infinite_canvas/project_detail`, `/mweb/v1/infinite_canvas/v1/get_canvas_custom_ratio`, and `/mweb/v1/infinite_canvas/get_conversation_list`; latest proof returned one canvas project, one detail record, zero custom ratios, and zero conversations. Remaining infinite-canvas conversation, project mutation, edit, changeset, delete, and custom-ratio update paths are explicitly blocked pending exact UI capture, real conversation ids, or disposable fixture/approval context. The `capcut-editor-catalog` command live-proves four no-spend LV editor catalog endpoints without browser foregrounding: `/lv/v1/effect/get_panel_info`, `/lv/v1/effect/get_category_effects`, `/lv/v1/effect/get_all_fonts`, and `/lv/v1/editor/plane/color/feed`; latest proof returned 14 categories, 745 font/effect rows, and 20 color palettes. Nine additional LV read-state endpoints are now blocked after no-spend signed probes: editor effect recent lists and EverPhoto state paths returned `ret=1015`, plane draft content map returned `ret=1016`, common recent list returned `ret=0` with empty `item_list`, preset resources returned `ret=-1 system busy`, and task multi-get returned `ret=1015`. The remaining CapCut template search/batch/preset endpoints are explicitly blocked with replay evidence and recommended action `capture_exact_payload`. Seven video-generation helper endpoints are also explicitly blocked pending exact UI payload capture: `/mweb/v1/video_generate/get_switch_model_queue_info`, `/mweb/v1/video_generate/pre_process`, `/mweb/v1/video_generate/mget_pre_process_result`, `/mweb/v1/video_generate/face_auth/skip`, `/mweb/v1/video_generate/face_auth/skip/query`, `/mweb/v1/aigc_draft/cancel_generate`, and `/mweb/v1/aigc_draft/generate_accelerate`. Ten CapCut/LV editor image helper endpoints are now blocked pending exact editor UI payload capture: `/lv/v1/editor/image/ai_model/submit_task`, `/lv/v1/editor/image/ai_model/batch_get_results`, `/lv/v1/editor/image/ai_model/materials`, `/lv/v1/editor/image/ai_model/create_cloth_mask`, `/lv/v1/editor/image/batch_get_url`, `/lv/v1/editor/image/embed_resource`, `/lv/v1/editor/image/gen_background`, `/lv/v1/editor/image/interactive_matting`, `/lv/v1/editor/image/saliency_seg`, and `/api/biz/v1/image/entity_seg`. Four LV asset reads are now blocked pending exact workspace/space/session context capture: `/lv/v1/asset/list`, `/lv/v1/asset/query`, `/lv/v1/asset/detail`, and `/lv/v1/asset/query_process`; safe `/lv/v1/asset/query` probes returned `ret=1014 system busy`. Supporting `/cc/v1/workspace/get_user_workspaces` probes with count/cursor and `lite_aid=513695` also returned `ret=1014`, but that namespace is outside the current static-inventory count. Six LV editor/template endpoints are now blocked pending exact auth or id context: `/lv/v1/cc_web/plane/del_presets_template` is a mutating delete path rejected by `capcut-probe`, `/lv/v1/editor/template/recent_list` and `/lv/v1/editor/template/check_post_permission` returned `ret=1015 check login error`, `/lv/v1/editor/draft/get_template_file` returned `ret=1016 ERR_PARAM` for empty `uris`, `/lv/v1/editor/plane/intelligence/query_recommend_template` returned `ret=-3 bad request` for a no-asset body, and `/lv/v2/cc_web_task/get_task_draft` returned `ret=1015 check login error` from the signed feed-api host. Fourteen LV asset/template/history mutation endpoints are now blocked without replay because they create, delete, rename, publish, sync, promote, remove, or otherwise mutate account/workspace state: `/lv/v1/asset/copy`, `/lv/v1/asset/create`, `/lv/v1/asset/create_cloud_asset`, `/lv/v1/asset/delete`, `/lv/v1/asset/label_as_exported`, `/lv/v1/asset/prepare_upload_cloud`, `/lv/v1/asset/rename`, `/lv/v1/editor/template/add`, `/lv/v1/editor/template/add_async`, `/lv/v1/editor/template/add_query`, `/lv/v1/ever_photo/batch_sync_asset`, `/lv/v1/ever_photo/promote_asset`, `/mweb/v1/remove_history`, and `/mweb/v1/update_video_default_bgm`. The LV `saliency_seg` endpoint is distinct from the implemented Jimeng `/mweb/v1/saliency_seg` mask command. Normalized proof files contain no credential markers.
...
4. Recover the `/mweb/search/v1/search` response transform, then expand template/research mining with CapCut template search, batch, and preset endpoints.
