...
- reference-profile research: `/mweb/v1/get_user_info`, `/mweb/v1/get_homepage`, `/mweb/v1/get_favorite_list`, `/mweb/v1/get_follow_list`, and `/mweb/v1/get_item_info` are implemented as no-spend `profile-research`; public profile reads use `sec_uid`, published work detail uses `published_item_id`, and follow/follower lists are current-account scoped
...
Current 2026-06-11 proof found 251 static frontend/API resources and counts `unknown=120`, `partial=4`, `implemented=50`, `dry_run_only=4`, `blocked=70`, `captured_only=2`, and `cataloged_only=1`. `profile-research` promoted five public/current-account social-reference reads and live-proved all six command modes in one sequential run: public profile, homepage, favorites, current-account following/followers, and published item detail. The normalized proof preserves prompts, model keys, first-frame provider URIs, dimensions, duration, and engagement counters without signed URL values. Proof is under `data/jimeng-lab/proof-20260611-profile-research-cli-{dry-run,live}/` and `data/jimeng-lab/proof-20260611-static-inventory-profile-research/`.
