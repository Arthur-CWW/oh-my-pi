...
| `multimodal2video` / omni-reference | dry-run and compare-gate proved in `jimeng-browser-proxy` | `omni-video-plan` builds the Seedance 2.0 `functionMode="omni_reference"` submit body with mixed image/video `unified_edit_input.material_list` and prompt-derived `meta_list`; `omni-video-compare` compares stable captured UI paths while ignoring generated UUIDs. This covers reference-profile/persona-swap motion transfer without live spend; live submit still needs passive capture compare and explicit approval. Proof: `docs/qa/jimeng-omni-video-plan-20260611.md`. |
...
| `local-items` | implemented in `jimeng-browser-proxy` | No-spend direct `/mweb/v1/get_local_item_list` current-account generated item detail by `item_id_list`. Latest proof returned 2 generated image rows with prompt, model, provider URI, dimensions, and author metadata; normalized output omits signed URL values. |
| `history-list` | implemented in `jimeng-browser-proxy` | No-spend direct `/mweb/v1/get_history` paginated list with `--limit`, `--offset`, `--direction`, `--workspaceId`, `--filter-types`, and `--order-by`. Empty `records_list` is a valid response; missing `records_list` is treated as contract drift. |
...
Current 2026-06-11 proof found 251 static frontend/API resources and counts `unknown=109`, `partial=4`, `implemented=61`, `dry_run_only=4`, `blocked=70`, `captured_only=2`, and `cataloged_only=1`. `profile-research` now covers seven public/current-account social-reference reads: public profile, homepage, favorites, stories, current-account following/followers, single published item detail, and batch published item detail. `local-items` covers current-account generated item detail from `/mweb/v1/get_local_item_list` by `item_id_list`, useful after `assets` returns generated item ids. `account-config` covers no-spend current-account settings, web-registration, and invite-status reads. `runtime-config` covers no-spend frontend experiment params, home banner config, helpdesk entrance, ASR token config, and ASR hotwords; normalized output fingerprints token and URL values instead of exposing them. The normalized proofs preserve prompts, model keys, first-frame provider URIs, dimensions, duration, story cover metadata, engagement counters, account settings, and runtime config counts without signed URL or token values. Proof is under `data/jimeng-lab/proof-20260611-profile-research-cli-{dry-run,live}/`, `data/jimeng-lab/proof-20260611-profile-research-stories-{dryrun,live}/`, `data/jimeng-lab/proof-20260611-profile-research-batch-items-{dryrun,live}/`, `data/jimeng-lab/proof-20260611-local-items-{dryrun,live}/`, `data/jimeng-lab/proof-20260611-account-config-{dryrun,live}/`, `data/jimeng-lab/proof-20260611-runtime-config-{dryrun,live}/`, and `data/jimeng-lab/proof-20260611-static-inventory-runtime-config/`. The superseded `ret=1000` proof at `data/jimeng-lab/proof-20260611-probe-mget-item-info/` used guessed field names; the successful probe at `data/jimeng-lab/proof-20260611-probe-mget-item-info-item-id-list/` recovered the frontend wire field `item_id_list`. The local item probe at `data/jimeng-lab/proof-20260611-probe-local-item-list-generated-items/` proved generated asset item ids work with `/mweb/v1/get_local_item_list`; account-config endpoint probes under `data/jimeng-lab/proof-20260611-probe-account-config-get-{settings,ug-info,invite-status}/` proved empty-body reads; runtime-config probes under `data/jimeng-lab/proof-20260611-probe-{home-header-banner,help-desk-entrance,experiment-params,asr-token,asr-hotwords}/` proved the current empty-body reads before CLI promotion.
...
4. Expand template/research mining with CapCut template search, batch, and preset endpoints.
