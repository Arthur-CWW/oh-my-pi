...
| `/mweb/search/v1/search` | POST | Full keyword search across inspiration, short-film, and asset channels. | Request builder recovered statically; response promotion remains partial pending frontend decrypt/sign transform |
...
- `/mweb/search/v1/sug` and `/mweb/search/v1/guess` are implemented as `research-keywords`; `/mweb/search/v1/search` is partial until the frontend result transform is recovered, and `/mweb/search/v1/fetch_debug/search` is a blocked debug wrapper.
