...
|---|---:|---|---|
| `/mweb/v1/workspace/list` | POST | Read current Jimeng workspace/project list for logged-in context. | Implemented as schema-backed no-spend `workspace-context`; static/dry-run proved, live replay pending local DNS recovery |
| `/mweb/v1/workspace/get_by_ids` | POST | Look up workspace metadata by ids inferred from the list or supplied explicitly. | Implemented as schema-backed no-spend `workspace-context`; request body uses snake_case `workspace_ids` |
...
Current schema-backed endpoints include `history-queue`, `history-records`, `video-info`, `agent-catalog`, `image-models`, `account-credit`, `commerce-benefits`, `workspace-context`, `text2image-plan`, and the envelope/timing layer used by `rate-probe`. If Jimeng changes `ret`/`errmsg`/`data` or the relied-on record/media/model/benefit/workspace/request paths, the CLI should fail with an explicit contract-changed error instead of silently normalizing stale shapes.
...
| Workspace context | `/mweb/v1/workspace/list` and `/mweb/v1/workspace/get_by_ids` are implemented as no-spend `workspace-context` for logged-in workspace/project metadata reads; workspace create/update/delete remain capture/disposable-fixture targets because they mutate account state |
