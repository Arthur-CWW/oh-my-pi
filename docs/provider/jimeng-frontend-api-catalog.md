...
- Public Jimeng reference-profile reads are implemented as `profile-research`: profile, homepage, favorites, and item detail preserve prompts/model keys/reference media metadata in normalized output without signed URL values; following/follower lists are labeled current-account scoped.
