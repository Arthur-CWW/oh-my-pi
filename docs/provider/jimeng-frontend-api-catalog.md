...
Latest no-spend `/mweb/v1/get_common_config` rate sweep completed `512/512` read-only requests at concurrency `256` with HTTP `200` and `ret=0`; no HTTP `429`, auth, or risk-control stop was observed on that endpoint. Treat this as a config-endpoint bound only; it is not a generation-submit limit. The external `iptag/jimeng-api` project does not publish a hard limit; it load-balances comma-separated bearer tokens randomly and uses polling/retry behavior.
...
| Account/quota reads | `/commerce/v1/benefits/user_credit`; implemented as signed no-spend `account-credit`, useful for checking quota before paid generation tests. `/commerce/v1/benefits/credit_receive` is a mutation and remains unimplemented without explicit approval. |
