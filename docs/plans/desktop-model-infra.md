...
**Addendum (2026-07-16, companion overnight-2, post-wrap):** program is wrapped (nested checkpoints `0a69024`/`93593ab`). Playground's steps 1–4 landed with byte-exact-prefix proof on `kinds.sh`, green daemon canaries, and pre-migration job-id resolution in `fetch` — accepted. **Step 5 is cleared**: one clean-cutover commit renaming companion kinds + updating both submitters (`scripts/gpu-queue.ts`, lab submit route in `apps/ai-companion-rtc/src/server.ts`) + docs, with a pre/post no-op `generic` job proof; post the diff on the bus for kernel-owner review (next companion session or Arthur if this session is parked).
