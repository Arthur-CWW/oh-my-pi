...
|---|---|---|---|---|---|---|---|
| meta | Meta-orchestrator loop | `Main` / `coord-asmr-seedance` | active | none |  | `agent://preflightblockers`; `docs/qa/asmr-companion-goal6-final-handoff-20260624.md` | Root orchestrator active; Goal Mode active; Codex-style OMP compaction configured globally; Goals 1–6 accepted |
...
| 2 | SynthID-conditioned Seedance I2V + access/preflight | `goal2seedanceResume` | accepted | none | `3f2b1b49` | `cd packages/jimeng-client && bun test ./test/seedance-image2video-plan.test.ts`; `cd packages/jimeng-client && bun run typecheck`; dry-run CLI proof under `data/asmr-companion/goal2/seedance-parent-proof/` | Emits `normalized/generated-video-clips.v1.json`, shared-contract first-frame conditioning sidecar, marked-conditioning dry-run proof under `data/asmr-companion/goal2/seedance-marked-conditioning-proof/`, and Goal 2 runbook `docs/qa/asmr-seedance-goal2-runbook-20260624.md`; live E2E remains gated by standing approval + preflight |
...
| 4 | Pleometric planning and pipeline handoff | `goal4planning` | accepted | none | `4e40f278`, `0a245e31` | `cd packages/pleometric-planner && bun run typecheck`; `cd packages/pleometric-planner && bun test ./test/planner.test.ts`; CLI proof under `data/asmr-companion/goal4-planning/goal4-pipeline-handoff.bundle.json` | Emits `goal4-pleometric-pipeline-handoff.v1` with internal provider/endpoint/DAG metadata for Goal 5 |
| 5 | TikTok/Jimeng/Remotion/HyperFrames integration | `goal5pipeline` | accepted | none | `8b529e61` | `bun workflows/tiktok-recreate/goal5-asmr-handoff.ts ... --outDir data/asmr-companion/goal5-pipeline-proof`; `bun test workflows/tiktok-recreate/goal5-asmr-handoff.test.ts`; `bun run remotion-renderer:render -- --manifest ...`; `bun run tiktok-recreate:hyperframes -- --layer-plan ...` | Real proof dir under `data/asmr-companion/goal5-pipeline-proof/`; reviewer HTML, visible placeholder SVG, Remotion MP4, HyperFrames project |
| 6 | Final integrated proof/handoff | `goal6proof` | accepted | none | `898d2686`, `7674f316` | `docs/qa/asmr-companion-goal6-final-handoff-20260624.md`; `docs/qa/asmr-companion-goal6-artifact-inventory-20260624.json` | reviewer handoff separates implemented, verified, dry-run/placeholder, and exploratory work |
