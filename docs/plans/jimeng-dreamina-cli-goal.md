...
- W4 started: `account-config`, `runtime-config`, `workspace-context`, `research-keywords`, `research-search`, `local-items`, `assets`, `history-queue`, `history-records`, and `video-info` now accept the shared Jimeng fetch transport and the CLI passes `--transport`/`--cassette` through to record/replay/fixture modes. Each service test records a cassette and replays it without re-hitting a provider; `workspace-context` covers the dependent list-to-get-by-ids path, `research-keywords` covers supported guesses while preserving the skipped asset-suggestion case, `research-search` covers the inspiration-search normalization path, `local-items` covers generated-item lookup, and the history/assets/video batch covers workbench asset list, queue state, completed history lookup, and VOD metadata.
