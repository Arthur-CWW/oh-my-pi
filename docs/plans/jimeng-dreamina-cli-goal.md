...
Keep going until the important/high-value UGC capabilities reachable from Arthur's logged-in Jimeng frontend are actually finished as typed CLI/client workflows with current proof, not merely cataloged. Lower-value or supporting endpoints may still end as blocked/skipped/unknown when that is the honest result, but the important APIs should be pushed to a working end-to-end state whenever the current frontend/provider allows it.

The goal is product-value driven, then coverage-driven. Choose the next work by the highest-value UGC workflow it unlocks, not by which endpoint is safest or cheapest to touch. Current direction from Arthur, 2026-06-13: spend credits when that is the direct path to proving the API/CLI; do not detour into low-value dry-run planning to avoid spend. Avoid ceremony that does not improve confidence. Do not require a local proof bundle for every backend refactor. Do not manually maintain walls of assertions when snapshot fixtures or structured endpoint registries would prove the same thing faster.
...
- Treat approved live generation and mutation as the default proof path for high-value workflow packets. Record the exact command, expected credit/account risk, and artifact path before running; then save raw/normalized responses, downloaded artifacts, and replayable contracts.
- Use dry-run planners only as compare gates, fixture generators, or a fallback when a provider action is genuinely unavailable. They are not a substitute for proving submit/poll/download when spend is approved.
...
- Prefer bounded live/capture matrices over manually adding more one-off dry-run planners.
...
3. Pick the highest-value unfinished packet from `triage-coverage --decisions keep`.
4. Create or refresh the packet manifest with useful examples, live/capture sample source, artifact directory, contracts to infer, files to promote, and acceptance commands.
5. For Arthur-approved live/spend waves, run the bounded matrix directly after logging exact commands/actions and expected risk; otherwise ask once rather than switching to a lower-value packet.
...
| W6 | Contract inference and scaffold generation | L | W5 samples; W1 transport | Yes, if output dirs are disjoint | Use `contract-infer` to turn saved JSON/cassettes into Effect Schema drafts, wrapper/CLI/test/registry patch drafts, replay fixtures, and Vitest snapshots. This should replace manual field-by-field schema writing for most new APIs. Improve the generator whenever a repeated manual step appears. |
| W7 | Promote generated scaffolds into typed client/CLI surface | L | W6 | Yes by API family | Hand-tighten required paths, flag semantics, redaction, and error handling, then land replay tests/snapshots. Work by family packet: generation, persona/voice, lip-sync, reference controls, or template mining. |
...
- W2/W6 continuation: `triage-coverage` emits endpoint-level value-ranked remaining work, while `selectJimengDiscoveryNextPacket()` now records the packet-level cutover and `pickNextJimengPacketId()` delegates to it. Endpoint gaps still expose direct signer/material-helper follow-ups, but packet selection marks `gen-parity` complete for useful user-facing workflows, keeps `persona-voice` blocked on disposable voice/subject mutation fixtures, and selects `lip-sync-human` as the active next packet. The next unambiguous capture command is `bun packages/jimeng-client/src/network-recorder.ts --cdp http://127.0.0.1:9340 --target-url "https://jimeng.jianying.com/ai-tool/digitalHuman?type=digitalHuman&workspace=undefined" --flow lip-sync-human-digitalhuman-submit --durationSec 120`.
- W6/G1 continuation: representative generation-plan Vitest snapshots cover three useful UGC workflows without spend: Korean beauty persona still (`text2image-plan`), faceless protein-bar hook video (`text2video-plan`), and reference-profile first/end-frame pose/timing transfer (`text2video-plan`). See `docs/qa/jimeng-generation-plan-snapshots-20260611.md`. These remain useful compare gates, but they no longer outrank the selected `lip-sync-human` capture packet now that user-facing generation parity is live-proven.
- W6/G1 continuation: `generate-audit-plan` now builds the dry-run `/mweb/v1/execute_generate_audit` material pre-audit body for image/video/audio/subject inputs and uses the generic `request-plan-compare` gate for passive UI capture parity. `executeJimengGenerateAudit` now promotes that request contract into a typed service helper with upstream-ret rejection, permissive material-result summaries, and shared transport cassette replay tests. See `docs/qa/jimeng-generate-audit-plan-20260611.md` and `docs/qa/jimeng-generate-audit-client-20260612.md`. Live replay remains blocked until a fresh frontend generation capture confirms the extra top-level fields for the actual submit context.
...
- W6/V1 continuation: `mix-audio-plan` now builds dry-run `/mweb/v1/mix_audio_video` and `/mweb/v1/mix_audio_videos` request plans for applying generated voice/audio to generated video items, and `executeJimengMixAudioVideo` promotes the single/batch request contract into typed service helpers with upstream-ret rejection, signed URL redaction, and shared transport cassette replay tests. It models the frontend transform that snake-cases the body and sends `babiParam` as query `babi_param`; `request-plan-compare` validates optional query params as well as POST body paths. See `docs/qa/jimeng-mix-audio-plan-20260611.md` and `docs/qa/jimeng-mix-audio-client-20260613.md`. Live replay remains blocked until passive UI capture compare passes and task-state mutation is approved.
...
- W6 contract-infer continuation: `jimeng-browser-proxy contract-infer` now turns saved proof/cassette directories into endpoint-grouped contract summaries, frequent path/type stats, artifact summaries, Effect Schema IR, registry patch drafts, and Vitest snapshotable output. First real scaffold output for `/mweb/v1/aigc_draft/generate` is under `data/jimeng-lab/proof-20260612-live-generation-matrix/contract-infer/normalized/contract/`. Next step is promoting those inferred generation paths into the hand-tightened typed generation client surface, then repeating the same matrix/infer/promote loop for persona/voice, lip-sync, reference controls, and template mining.
- W6 generation-contract continuation: `jimeng-browser-proxy generation-contract` hand-tightens saved live `/mweb/v1/aigc_draft/generate` proof result JSON into an Effect Schema-backed submit/poll/artifact summary contract. The first replay over `data/jimeng-lab/proof-20260612-live-generation-matrix/` validated 4 saved video generation proofs and wrote reports under `data/jimeng-lab/proof-20260612-live-generation-matrix/generation-contract/normalized/generation-contract/`. `executeJimengVideoDirectSubmit` and `executeJimengText2ImageSubmit` now add typed no-live-spend direct submit helpers around the existing plan builders; see `docs/qa/jimeng-generation-direct-submit-client-20260613.md`. Next high-value loop should repeat matrix/infer/promote for persona/voice, lip-sync/digital-human, reference controls, or template mining rather than adding more low-value dry-run planners.
- W6/L1 continuation: `video-preprocess-plan` and `video-preprocess-query-plan` now model the dry-run `/mweb/v1/video_generate/pre_process` and `/mweb/v1/video_generate/mget_pre_process_result` request shapes for avatar image checks, voice recommendation, audio detect, audio silence, and result lookup. See `docs/qa/jimeng-video-preprocess-plan-20260612.md`. Live replay remains blocked until passive UI capture compare passes and task-state mutation is approved.
- W6/L1 continuation: `contract-infer` now recognizes nested `plan.endpoint` dry-run plans and emits useful lip-sync/digital-human flag suggestions for image/avatar lip-sync, VOD lip-sync, pre-process submit, and pre-process result query. The proof run under `data/jimeng-lab/proof-20260612-lip-sync-human-contract-infer/` generated scaffolds for `/mweb/v1/aigc_draft/generate`, `/mweb/v1/video_generate/pre_process`, and `/mweb/v1/video_generate/mget_pre_process_result`; see `docs/qa/jimeng-lip-sync-human-contract-infer-20260612.md`. The remaining L1 gap is passive UI capture/compare or approved live submit/poll/download for a talking-head artifact, not offline schema scaffolding.
- W6/L1 continuation: `submitJimengVideoPreprocess` and `fetchJimengVideoPreprocessResults` now promote the pre-process submit/result paths into typed service helpers with shared transport/cassette replay coverage. See `docs/qa/jimeng-lip-sync-human-preprocess-client-20260612.md`. Live provider calls remain approval-gated because pre-process creates provider task state and result lookup needs captured or approved task ids.
