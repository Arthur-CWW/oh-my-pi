...
- W2 started: static-inventory coverage tests were rewritten from a large wall of one-off assertions into explicit fixture endpoint arrays plus table-driven status/command/action/risk expectations. The underlying endpoint registry still lives in `discovery-worklist.ts`; extracting it into a dedicated registry module remains the next W2 cleanup.
