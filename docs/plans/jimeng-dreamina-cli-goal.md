...
- W2 started: static-inventory coverage tests were rewritten from a large wall of one-off assertions into explicit fixture endpoint arrays plus table-driven status/command/action/risk expectations. Endpoint ownership now lives in `endpoint-registry.ts`; the next W2 cleanup is a registry/triage coverage snapshot that maps keep families to endpoint statuses.
