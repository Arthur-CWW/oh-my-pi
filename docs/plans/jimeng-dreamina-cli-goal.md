...
- W4 started: `account-config`, `runtime-config`, and `workspace-context` now accept the shared Jimeng fetch transport and the CLI passes `--transport`/`--cassette` through to record/replay/fixture modes. Each service test records a cassette and replays it without re-hitting a provider; `workspace-context` covers the dependent list-to-get-by-ids path.
