...
- Workflow telemetry/import contract: `workflowEvents` is append-only, `workflowRuns` is the durable derived snapshot, core read/write routes include `GET/POST /api/ugc/workflows`, `POST /api/ugc/workflows/demo`, `GET/POST /api/ugc/workflows/<run_id>/events`, and `POST /api/ugc/workflows/<run_id>/import`; the browser stream route is polling-backed SSE at `GET /api/ugc/workflows/events/stream`; dynamic-workflow callback telemetry maps to literal `phase`/`message`/`started` events. The demo launcher surface is the workflow-panel header `Deterministic local demo workflow launcher`, buttons `Run brainrot demo`, `Run UGC ads demo`, and `Run both demos`, plus `bun run demo:workflows [brainrot|ugc-ads|all]` from `apps/slotok-workbench`; it is deterministic clean-room local planning only, not Pi/OMP RPC or live provider execution. The launcher appends a `completed` event but the durable run snapshot ends at `status: succeeded` with `currentPhase: "completed"`. Handoff import is dry-run by default (`200`) and applies only via `apply: true` (`201`), with clean-room `sourcePolicy` guardrails, unknown-key/id rejection, provider-job/reference-archive/candidate-note import behavior, returned `plannedChanges`/`workflowRun`/`events`, and credential-like key redaction. OMP RPC, Pi/OMP artifact polling, and direct dynamic workflow execution are future adapters only; OMP stats are historical usage data only.
...
- [ ] Parent verifies `GET/POST /api/ugc/workflows` exposes run snapshots, `GET /api/ugc/workflows?lane=ugc-ads&status=running&limit=25` filters snapshots, `POST /api/ugc/workflows/demo` accepts `{ lane: "brainrot" | "ugc-ads" | "all" }`, and `GET/POST /api/ugc/workflows/<run_id>/events` reads/appends event history with lane, status, current phase, counters, imported record ids, artifact paths, timestamps, result/error summaries, and source metadata available in the derived run state.
- [ ] Parent verifies the merged browser UI exposes the header `Deterministic local demo workflow launcher` plus buttons `Run brainrot demo`, `Run UGC ads demo`, and `Run both demos`, with copy that clearly says deterministic clean-room local planning only, not Pi/OMP RPC, direct dynamic workflow execution, or live provider execution.
- [ ] Parent verifies `bun run demo:workflows [brainrot|ugc-ads|all]` from `apps/slotok-workbench` targets the running daemon and creates the same browser-visible workflow runs as the UI route.
- [ ] Parent verifies clicking each demo button creates completed local run timelines plus visible result JSON and persisted imported records after refresh/reload, while the durable run snapshot lands on `status: succeeded` with `currentPhase: "completed"`.
...
- [ ] Parent verifies `POST /api/ugc/workflows/<run_id>/import` accepts only top-level `payload` and `apply`; `Sample payload` loads the metadata-only local demo handoff, `Dry-run validate` returns HTTP `200` with `schemaVersion: "ugc-studio.workflow-import-result.v1"`, `dryRun: true`, `plannedChanges`, `workflowRun: null`, and `events: []`, and valid `Apply import` returns HTTP `201`, imports safe provider jobs/reference archives/notes/candidate status-note patches, appends `import`/`result` events, and updates run imported ids/artifacts/result.
