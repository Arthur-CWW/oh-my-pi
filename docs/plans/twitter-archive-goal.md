...
| Authenticated browser capture | Preferred Firefox WebExtension posts read-only visible tweet/bookmark/profile-card captures to the local dev server at `http://127.0.0.1:3420`; the Violentmonkey userscript can send the same lightweight `pageUrl`/`visibleTweets` payload as a fallback, and the existing `x-bookmark-sync-devtools` extension remains a valid dedicated Chrome/Chromium/Helium DevTools network-capture lane. |
