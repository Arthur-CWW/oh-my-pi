...
- Browser bookmark import has two separate local lanes: Chrome/Arc `Bookmarks` JSON and Firefox `places.sqlite` may seed public profile/status/search scrape candidates with provenance, and the user-installed DevTools extension may capture bookmark/network responses while the user is logged in and actively viewing X. Neither lane stores cookies/tokens or performs Twitter/X mutations.
...
| Wayback/public archive imports | Import/list public Internet Archive CDX/Wayback Twitter/X snapshots and user-provided/local archives into the same SQLite tables with source metadata. |
...
| Bookmark DevTools sync | Existing `x-bookmark-sync-devtools` extension observes X/Twitter DevTools network responses while the user is logged in and viewing/scrolling, then POSTs captured response payloads to the local dev server at `http://127.0.0.1:3420`; the server owns SQLite/Markdown writes. |
...
- SQLite contains resumable runs/jobs, raw page cache, normalized users/tweets/media, shared raw/parse/media/thread/quote/import/skip statuses, media local paths when downloaded, quote resolution fields, Wayback/import batch metadata, following graph edges, provenance, local notes/tags/marks, and run events.
...
- Extension safety is visible to reviewers: DevTools/network capture runs only while the user has X open and is scrolling/visiting pages, saves no cookies/tokens, mutates nothing on X, and posts only to localhost.
