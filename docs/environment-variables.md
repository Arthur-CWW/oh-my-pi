...
| `OMP_WORKSTREAM`             | Classifies new root sessions with a workstream slug or `adhoc`. Ignored when resuming; CLI `--workstream` takes precedence and may explicitly reclassify a resumed session. Exact launch directories named `streams/<slug>` are inferred only when this variable is empty. |
