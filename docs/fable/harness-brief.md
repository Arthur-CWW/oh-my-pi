...
- **Route through the doctrine.** [`routing-doctrine.md`](routing-doctrine.md) is the policy source: its constraints, precedence, eligible-lane selection, and resolved-decision record govern every route. Model/tool fit and refusal basins are dated, evidence-linked observations in its ledger, not fixed mappings in this brief.
...
- **Plan for succession.** The frontier orchestrator rotates. Everything durable must be model-agnostic: charters and skills address the frontier orchestrator, while the routing doctrine separates stable policy from changing lane facts and resolved decisions.
...
3. **Dynamic review rules** — things no static rule can express: taste, architecture drift, silent scope-shrink. Encoded as reviewer-lane prompts with a named checklist per stream, run at phase gates, not continuously.
...
```mermaid
flowchart TD
  A[Arthur: goals, rants, preferences, failures] --> B[Frontier orchestrator]
  B --> C[Routing doctrine: constraints, eligible lanes, resolved decision]
  C --> D[Execution lanes]
  C --> E[Ambient retrieval: Kagi, X archive, history, vault, sessions]
  D --> F[Proof artifacts: tests, screenshots, logs, diffs]
  F --> G[Dreaming loop: observations -> candidate harness patches]
  G --> H[Review gate: frontier orchestrator / Arthur]
  H --> C
```
...
3. Propose small patches: disable a default tool, add a dormant skill, move prompt text into a skill, update routing observations or policy through the doctrine, add an index.
4. **Review gate before anything lands.** The loop proposes; the frontier orchestrator and Arthur dispose.
...
Current-system map and staged control-plane direction: [`agent-system-overview.md`](agent-system-overview.md).

1. Routing decision layer — apply the doctrine's precedence and constraints to produce legible resolved decisions. Next meta slice.
2. Skill-leak consolidation (multi-source discovery → one intentional profile). *In progress 2026-07-03.*
3. Per-agent skill/tool exposure — lanes declare what they carry; default shrinks.
4. Orchestrator UI — watch agent progress live. Substrate: the live control-plane ledger and publisher outbox, with typed session JSONL, Bun+SSE dashboard pattern in `packages/jimeng-client/src/artifact-dashboard.ts`, and React19+Vite+shadcn shell in `apps/slotok-workbench`.
5. Packet contract as a first-class template for dispatches.
6. Friction log surface (cheap: a `docs/state/harness-friction.md` or SQLite table) feeding the dreaming loop.
7. Dreaming loop prototype, gated.
