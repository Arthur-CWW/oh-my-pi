...
| `.omp/fable-config.yml` | Fable session lane resolution and harness skills | Sol-high default/task/plan, Luna-xhigh smol, explicit designer/vision; 80-request soft budget. Current operational posture, not doctrine. |
| `.omp/{companion,playground,primer}-config.yml` | Stream overlays and stream-local skill exposure | Sol-high orchestrator, Terra-medium task/smol, Sol-high slow/plan, explicit designer/vision; 40-request soft budget. Primer currently differs on designer (`claude-opus-4-8` vs `4-6`); treat as visible drift/question, not a mechanical fix. |
...
- Current routing posture is explicit: **Fable owns intent, ontology, architecture, synthesis, and taste. Luna xhigh is the default bounded implementer** for UI adjustments, fixtures, configuration, docs, inventories, adapters, data extraction, and proof runbooks. **Sol is an escalation lane** for concurrency, persistence, process lifecycle, crash recovery, security boundaries, difficult cross-module integration, and independent review—not the default merely because a task edits production code.
...
- Dated posture (Arthur, 2026-07-13): observed partial order among cheap lanes — **Luna > other weak lanes** (Kimi, Opus-cheap, Gemini); weak-lane usage is partly use-up-the-subscription economics, not merit. **Gemini 3.5 Pro/Flash (Antigravity sub) is reserved for video understanding/decomposition** — the parked playground video task claims it. Tiny scheduled intelligence-injection daemons (tab renamers, categorizers): try Luna-mini-class first and measure. Route on the lane-strength criterion (entropy × blast radius × mistake legibility — routing-doctrine.md §minimum-effective-cost), never task size.
- Dated posture (Arthur, 2026-07-15, voice): **never route to Terra — "Terra is not pareto-efficient at anything."** Workloads split two ways only: bounded → Luna xhigh+; synthesis/taste/architecture/retrieval-with-judgment → Sol medium+. Terra's former retrieval/bounded-logic lane folds into Luna xhigh. Global modelRoles (research, authenticated_web, plan) were repointed the same day.
- Dated posture (Arthur, 2026-07-15, follow-up): **the catch-all `task` role is an ontology mismatch** — "task is too general, we use it for everything." Spawns should name a responsibility role from the list above (scout/researcher, implementer, reviewer, QA/proof runner, operator, designer, synthesizer/integrator), each resolving to its own lane via the lane-strength criterion; a subagent needing a scout gets Luna-class regardless of its parent's lane. Until HR-122 lands, orchestrators approximate this with per-spawn model overrides (Luna xhigh for bounded/scout, Sol medium+ escalation with recorded reason) and `quick_task`/`explore` for smol lanes.
